...
public class PushGCM {
	...
	public static void sendC(String nodeid, String remoteid, ContextEvent event, String toURI) throws PushException {
		...
		StringBuilder combined = new StringBuilder(serial).append(subject).append(predicate).append(object)
				.append(ContextEvent.PROP_RDF_SUBJECT).append(ContextEvent.PROP_RDF_PREDICATE)
				.append(ContextEvent.PROP_RDF_OBJECT).append("method=SENDC").append("to=").append(toUri);
		...
		if (ttl > 0)
			build.timeToLive(ttl);
		// Payload data, expressed as parameters prefixed with data. and
		// suffixed as the key
		build.addData(RemoteAPI.KEY_METHOD, "SENDC").addData(RemoteAPI.KEY_TO, toUri)
				.addData(ContextEvent.PROP_RDF_SUBJECT, subject).addData(ContextEvent.PROP_RDF_PREDICATE, predicate)
				.addData(ContextEvent.PROP_RDF_OBJECT, object);
		// There is no limit on the number of key/value pairs, though there is a
		// limit on the total size of the message (4kb)
		try {
		...
		if (size < 4000) { // If >4k, dont send the serial, just the SpO (4000
							// to round up)
	...
	public static ServiceResponse callS(String nodeid, String remoteid, ServiceCall call, String toURI)
		...
		try {
			...
			if (inputs != null) {
				for (Iterator i = inputs.iterator(); i.hasNext();) {
					...
					if (in != null) {
						build.addData(in.getURI(),
								crypt ? CryptUtil.encrypt(call.getInputValue(in.getURI()).toString(), secretkey)
										: call.getInputValue(in.getURI()).toString());
						combined.append(in.getURI())
								.append(crypt ? CryptUtil.encrypt(call.getInputValue(in.getURI()).toString(), secretkey)
										: call.getInputValue(in.getURI()).toString());
					}
				}
			}

			// Add the call URI so the remote node can assign its response to
			// the call
			build.addData(RemoteAPI.KEY_CALL, crypt ? CryptUtil.encrypt(call.getURI(), secretkey) : call.getURI());
			combined.append(RemoteAPI.KEY_CALL)
					.append(crypt ? CryptUtil.encrypt(call.getURI(), secretkey) : call.getURI());

			// There is no limit on the number of key/value pairs, though there
			// is a limit on the total size of the message (4kb)
			try {
				size = combined.toString().getBytes("UTF-8").length;
			} catch (UnsupportedEncodingException e) {
				size = serial.length() * 4; // worst case, 4 bytes for all chars
			...
			if ((size + serial.length() * 4) < 4000) { // If basic params + the
														// serial still <4k,
														// send the full serial
														// too
				try {
		...
		pendingCalls.put(callID, sr); // For multitenancy, where 1 call can
										// address N nodes
		// Do it before sending to avoid getting answer before putting the
		// pendingCall
	...
	private static void send(String nodeid, String remoteid, Message msg) throws PushException {
		...
		if (remoteid.equals(REG_ID_OUTDATED)) {// See below for when this
												// happens
		...
		if (remoteid.startsWith("gcm:")) {// Remove the gcm: prefix from
											// remoteid. DEPRECATED
		...
		try {
			...
			if (id != null) {
				...
				if (canon != null) {
					Activator.getRemoteAPI().register(nodeid, canon); // This
																		// just
																		// updates
																		// the
																		// remoteid
				...
				if (res.getErrorCodeName().equals("NotRegistered")) {
					// Remote node app was
					// uninstalled/updated/unregistered/refreshed by Google
					// Maintain the MW wrappers, but dont allow push messages
					// until remote node updates itself
					Activator.getRemoteAPI().register(nodeid, REG_ID_OUTDATED); // This
																				// just
																				// updates
																				// the
																				// remoteid
	...
	public static void handleResponse(String resp, String nodeid) throws PushException {
		...
		try {
			...
			while (line != null && !line.equals(RemoteAPI.FLAG_TURTLE)) {
				String[] parts = line.split("=", 2);// parts[0]: key (output
													// URI) parts[1]: value
				if (parts.length == 2) {
					if (!parts[0].equals(RemoteAPI.KEY_STATUS) && !parts[0].equals(RemoteAPI.KEY_CALL)) {
						String[] resource = parts[1].split("@", 2);// resource[0]:
																	// uri
																	// resource[1]:
																	// type
						...
						key = parts[1] + "@" + nodeid;// Identifies the call in
														// pendingCalls
						...
						if (!parts[1].equals(CallStatus.succeeded)) {
							...
							// do not break the while because we should keep
							// reading lines so we get to serialized sr (better)
						}
					}
				}
				line = br.readLine();
			...
			while (line != null) {
				// We only get here if there was something after TURTLE (and
				// there was TURTLE)
				line = br.readLine();
				if (line != null)
					strb.append(line);
	...
	private static void notifyResponse(String key, ServiceResponse sr) {
		...
		if (lock != null) {
			synchronized (lock) {
				...
				Activator.logD("PushGCM.handleResponse",
						"UPDATING RESPONSE: " + key + ", " + sr.getCallStatus().toString());
				lock.notify();
			}
		}
	}
