...
public class Axis1ParserWrapper {
			...
			try {
				if (axis1Parser.getCurrentDefinition().getServices() != null) {
					...
					while (iter1.hasNext()) {
						...
						if (portsMap.size() != 1) {
							// theParsedDefinition.containingErrors.add("ERROR!
							// the service '"+serv.getQName().getLocalPart()
							// +"' contains "+portsMap.size()+" ports!
							// ....!=1");
						}
						...
						if (!soapPortFound) {
							theParsedDefinition.getContainingErrors()
									.add("ERROR! the service '" + serv.getQName().getLocalPart()
											+ "' did not contain SOAP PORT!!!  (contains " + portsMap.size()
											+ " ports!  ....!=1)");
						}
					}
				}
		...
		if (definedElement.getContainedElements() != null) {
			if (definedElement.getContainedElements().size() > 0) {
				...
				if (definedElement.getRefType() != null) {
					System.out.println(definedElement.getRefType().getQName().getLocalPart());// Type
																								// tou
																								// CO
					// co.setObjectType(definedElement.getRefType().getQName().getLocalPart()+"[]");
					System.out.println(definedElement.getRefType().isBaseType());// apo
																					// dw
																					// tha
																					// katalavw
																					// an
					...
					if (definedElement.getRefType().isBaseType()) {
						...
						if (definedElement.getRefType().getClass().getName()
							...
							System.out.println();
								...
								.contains("org.apache.axis.wsdl.symbolTable.UndefinedType")) {
		...
		if (definedType.getContainedElements() != null) {
			if (definedType.getContainedElements().size() > 0) {
					...
					if (containedElement.getMaxOccursIsUnbounded() && definedType.getContainedElements().size() == 1) {
						...
						if (containedElement.getType() != null) {
							if (containedElement.getType().isBaseType()) {
								...
								/*
								 * if(definedType.getQName().getLocalPart().
								 * equals ("GeoPlaceType")){
								 * System.out.println(""); }
								 */
											...
											if (childNode.getChildNodes() != null) {
												...
												for (int i1 = 0; i1 < childNode.getChildNodes().getLength(); i1++) {
													...
													if (restrictionNode != null && restrictionNode.getNodeName() != null
														...
														if (restrictionNode.getAttributes() != null && restrictionNode
																.getAttributes().getNamedItem("value") != null) {
															System.out.println(restrictionNode.getAttributes()
																	.getNamedItem("value"));
															no1.getHasAllowedValues().add(restrictionNode
				...
				if (definedType.getRefType() != null) {
					...
					System.out.println(definedType.getRefType().getQName().getLocalPart());// Type
																							// tou
																							// CO
					...
					if (definedType.getRefType().isBaseType()) {
						...
						// ComplexObject
						if (definedType.getRefType().getClass().getName()
								.contains("org.apache.axis.wsdl.symbolTable.DefinedType")) {
							ComplexObject co1 = new ComplexObject();
							...
							co.setIsArrayType(true);
							System.out.println();
						} else if (definedType.getRefType().getClass().getName()
								.contains("org.apache.axis.wsdl.symbolTable.UndefinedType")) {
					...
					// theParsedDefinition.getContainingErrors().add("ERROR
					// 123456!!! @line ~381");
					...
					if (t != null && t.getNode() != null && t.getNode().getAttributes() != null) {
						...
						if (isAbstractNode != null && isAbstractNode.getNodeValue() != null
								&& isAbstractNode.getNodeValue().equalsIgnoreCase("true")) {
	...
	private static void parseTheWSDLOperations(SymbolTable theSymbolTable, Definition theAxisDefinition,
			ParsedWSDLDefinition theParsedDefinition) {
		...
		while (portTypesIterator.hasNext()) {
			...
			System.out.println("Port Type name: " + portType.getQName().toString());// +"
																					// class:
																					// "+p.getBinding().getPortType().getQName().toString());//+"
				...
				while (operationsIter.hasNext()) {
					...
					while (bindingsIter.hasNext()) {
						...
						if (binding.getQName().getLocalPart().contains("Http")
								|| binding.getQName().getLocalPart().contains("HTTP")) {
							continue;
						}
					...
					System.out.println(operationInput.getMessage().getQName().toString() + "    "
							...
							while (inputPartsIterator.hasNext()) {
								...
								if (inputPart.getTypeName() != null) {
									...
									if (typeGotFromTheSymbolTable != null) {
										...
										if (inputPart.getElementName() != null) {
											nativeOrComplexObject = parseTypeGotFromTheSymbolTable(
													typeGotFromTheSymbolTable, theSymbolTable,
											...
											nativeOrComplexObject = parseTypeGotFromTheSymbolTable(
													typeGotFromTheSymbolTable, theSymbolTable, inputPart.getTypeName(),
													theParsedDefinition);
										}
										if (nativeOrComplexObject != null) {
									...
									if (typeGotFromTheSymbolTable != null) {
										...
										Object nativeOrComplexObject = parseTypeGotFromTheSymbolTable(
												typeGotFromTheSymbolTable, theSymbolTable, inputPart.getElementName(),
												theParsedDefinition);
										if (nativeOrComplexObject != null) {
											mitsosOperationInput.getHasNativeOrComplexObjects()
													.add(nativeOrComplexObject);
										...
										Element elementGotFromTheSymbolTable = theSymbolTable
												.getElement(inputPart.getElementName());
										if (elementGotFromTheSymbolTable != null) {
											Object nativeOrComplexObject = parseElementGotFromTheSymbolTable(
													elementGotFromTheSymbolTable, theSymbolTable,
													inputPart.getElementName(), theParsedDefinition);
					...
					if (outputMessageOfOperation.getParts() != null && outputMessageOfOperation.getParts().size() > 0) {
						...
						if (outputPartsCol != null) {
							...
							while (outputPartsIterator.hasNext()) {
								...
								// System.out.println("\t\t"+outputPart.getName()+"
								...
								if (outputPart.getTypeName() != null) {
									...
									if (typeGotFromTheSymbolTable != null) {
										...
										if (outputPart.getElementName() != null) {
											...
											nativeOrComplexObject = parseTypeGotFromTheSymbolTable(
													typeGotFromTheSymbolTable, theSymbolTable, outputPart.getTypeName(),
													theParsedDefinition);
										}
									...
									// LITERAL TYPE!!!
									Type typeGotFromTheSymbolTable = theSymbolTable
											.getType(outputPart.getElementName());

									if (typeGotFromTheSymbolTable != null) {

										Object nativeOrComplexObject = parseTypeGotFromTheSymbolTable(
												typeGotFromTheSymbolTable, theSymbolTable, outputPart.getElementName(),
												theParsedDefinition);
										if (nativeOrComplexObject != null) {
											mitsosOperationOutput.getHasNativeOrComplexObjects()
													.add(nativeOrComplexObject);
										...
										Element elementGotFromTheSymbolTable = theSymbolTable
												.getElement(outputPart.getElementName());
										if (elementGotFromTheSymbolTable != null) {
											Object nativeOrComplexObject = parseElementGotFromTheSymbolTable(
													elementGotFromTheSymbolTable, theSymbolTable,
													outputPart.getElementName(), theParsedDefinition);
	...
	private static Object parseElementGotFromTheSymbolTable(Element elementGotFromTheSymbolTable,
			SymbolTable theSymbolTable, QName objectName, ParsedWSDLDefinition theParsedDefinition) {
		if (elementGotFromTheSymbolTable == null) {
			return null;
		}
	...
	private static Object parseTypeGotFromTheSymbolTable(Type typeGotFromTheSymbolTable, SymbolTable theSymbolTable,
