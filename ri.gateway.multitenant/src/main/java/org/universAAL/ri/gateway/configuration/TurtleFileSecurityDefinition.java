...
public class TurtleFileSecurityDefinition implements OperationChainManager {
	...
	try {
	    ...
	    if (s.hasNext()) {
		String src = s.next();
		def = (Resource) Gateway.getInstance().serializer.getObject()
			.deserialize(src);
	    } else {
	    ...
	    throw new RuntimeException("URL: " + url.toString()
		    + " cannot be loaded for security definition", e);
    ...
    private class RegChecker extends Match implements
	...
	private boolean checkB(Resource[] res) {
	    ...
	    try {
		return policy == isSubset(new ArrayList<Resource>(
			Arrays.asList(res)));
	    } catch (Exception e) {
		LogUtils.logError(Gateway.getInstance().context, getClass(),
			"checkB", new String[] {
				"unable to determine if subset.",
				" Assuming not" }, e);
		return false;
	    }
	...
	public OperationResult check(Resource[] params) {
	    return checkB(params) ? OperationResult.ALLOW
	...
	public OperationResult check(ScopedResource message) {
	    return checkB(message) ? OperationResult.ALLOW
		    : OperationResult.DENY;
	}
	...
	if (match instanceof Resource
		&& ((Resource) match).getURI().equals(
			"http://www.w3.org/1999/02/22-rdf-syntax-ns#nil"))
	    match = Collections.EMPTY_LIST;
	if (match instanceof Matchable)
	    return new RegChecker((Matchable) match,
		    policy.contains("Whitelist"));
	...
	if (match instanceof Resource
	    ...
	    match = Collections.EMPTY_LIST;
	if (match instanceof Matchable)
	    return new MessageChecker((Matchable) match,
		    policy.contains("Whitelist"));
	if (match instanceof List)
	    return new MessageChecker((List) match,
		    policy.contains("Whitelist"));
	LogUtils.logWarn(Gateway.getInstance().context, getClass(),
