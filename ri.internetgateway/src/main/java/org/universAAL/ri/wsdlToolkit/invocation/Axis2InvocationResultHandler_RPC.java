...
public class Axis2InvocationResultHandler_RPC {
				...
				if (!outCO.isIsArrayType()) {
					...
					while (omElement.hasNext()) {
						if (((ComplexObject) outCO).getHasComplexObjects().size() > 0) {
							if (i == 0) {
								setComplexObjectValues((ComplexObject) outCO.getHasComplexObjects().get(0),
										omElement.next());
							} else {
							...
							if (i == 0) {
								setNativeObjectValue((NativeObject) outCO.getHasNativeObjects().get(0),
										omElement.next());
							} else {
	...
	private static Iterator findTheArrayObjectNodesInTheResponseBody(MessageContext inMsgCtx, NativeObject no,
			ComplexObject co) {
		if (inMsgCtx == null || inMsgCtx.getEnvelope() == null || inMsgCtx.getEnvelope().getBody() == null) {
			return null;
		}
	...
	private static Iterator<OMElementImpl> findTheArrayObjectNodesInOMElement(OMElementImpl omElement, NativeObject no,
			ComplexObject co) {
