...
public class ComplexObject /*implements Cloneable*/ {
    ...
    public ComplexObject cloneTheCO(){
        ...
        newCO.objectName=new QName(this.objectName.getNamespaceURI(), this.objectName.getLocalPart(), this.objectName.getPrefix());
        newCO.objectType=new QName(this.objectType.getNamespaceURI(), this.objectType.getLocalPart(), this.objectType.getPrefix());
...
/*
        if(!this.getObjectType().getLocalPart().contains("XmlSchemaChoice")){
            if(ITIWSDLParser.clonedTypes.containsKey(this.getObjectType())){
                Integer countSoFar=(Integer)ITIWSDLParser.clonedTypes.get(this.getObjectType());
                countSoFar++;
                ITIWSDLParser.clonedTypes.put(this.getObjectType(),countSoFar);
            }else{
                ITIWSDLParser.clonedTypes.put(this.getObjectType(),1);
            }
        }
...
*/
        ...
        while(iter1.hasNext()){
            ...
            if(checkIfCOhasParentTheSameCO(co)){
                ...
                co1.objectName=new QName(co.objectName.getNamespaceURI(), co.objectName.getLocalPart(), co.objectName.getPrefix());
                co1.objectType=new QName(co.objectType.getNamespaceURI(), co.objectType.getLocalPart(), co.objectType.getPrefix());
                ...
                for(int i=0;i<co.getHasExtendedObjects().size();i++){
                    ...
                    if(obj.getClass().getName().contains("ComplexObject")){
                        ...
                        extCO.objectName=new QName(inExtCO.objectName.getNamespaceURI(), inExtCO.objectName.getLocalPart(), inExtCO.objectName.getPrefix());
                        extCO.objectType=new QName(inExtCO.objectType.getNamespaceURI(), inExtCO.objectType.getLocalPart(), inExtCO.objectType.getPrefix());
                ...
                /*
                if(co.objectType.getLocalPart().contains("EnvelopeType")){//RS_Identifier
                    Object parent1=this.getHasParent();
                    Vector parentNames=new Vector();
                    while(parent1!=null){
                        //System.out.println(((ComplexObject)parent1).getObjectName().getLocalPart());
                        parent1=((ComplexObject)parent1).getHasParent();
                        if(parent1!=null){
                            parentNames.insertElementAt(((ComplexObject)parent1).getObjectName().getLocalPart(),0);
                        }
                    }
        ...
        while(iter3.hasNext()){
            ...
            if(obj.getClass().getName().indexOf("NativeObject") > -1){
                ...
                co.setHasParent(this);
                /*
                if(co.getHasComplexObjects().size()>0&&ITIWSDLParser.checkIfCOhasAlreadyBeenCloned(co.getObjectType())){
                    System.out.println("EXT:  "+co.getObjectName().getLocalPart()+"\t"+co.getObjectType().getLocalPart());
                }*/
                //if(ITIWSDLParser.checkIfCOhasAlreadyBeenCloned(co.getObjectType())||checkIfCOhasParentTheSameCO(co)){
                if(checkIfCOhasParentTheSameCO(co)){
                    ComplexObject co1=new ComplexObject();
    ...
    public String toString(){
        String result=this.getObjectName().getLocalPart()+
                "  {"+this.getObjectType().getLocalPart()+"}    NS="+this.getObjectType().getNamespaceURI();
        return result;
    }
