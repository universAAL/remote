...
public class GatewayPortTracker extends ServiceTracker implements Runnable {
	...
	public GatewayPortTracker(BundleContext bc, ModuleContext mcontext, String clazz, ServiceTrackerCustomizer stc) {
		super(bc, clazz, stc);
	...
	public synchronized void registerService(ServiceReference reference) throws ServletException, NamespaceException {
		GatewayPort gp = (GatewayPort) bc.getService(reference);
		gp.setContext(moduleContext);
		HttpContext servletContext = new GatewayHttpContext(reference);
		String url = gp.url();
		String dataDir = gp.dataDir();

		LogUtils.logInfo(Activator.mcontext, this.getClass(), "registerService",
				new Object[] { "dataDir->  {}", dataDir }, null);
