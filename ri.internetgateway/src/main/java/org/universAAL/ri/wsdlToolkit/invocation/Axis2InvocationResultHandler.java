...
public class Axis2InvocationResultHandler {
		...
		while (parsedOperationOutputsIter.hasNext()) {
			...
			if (parsedOutObj instanceof NativeObject) {
				...
				if (!outCO.isIsArrayType()) {
					...
					Vector<OMElementImpl> omElement = findTheArrayObjectNodesInTheResponseBody(
							inMsgCtx, null, outCO);

					for (int i = 0; i < omElement.size(); i++) {

						if (((ComplexObject) outCO).getHasComplexObjects()
								.size() > 0) {
							if (i == 0) {
								setComplexObjectValues((ComplexObject) outCO
										.getHasComplexObjects().get(0),
										omElement.get(i));
							} else {
								ComplexObject co = ((ComplexObject) outCO
										.getHasComplexObjects().get(0))
										.cloneTheCO();
								clearArrayTypes(co);
								clearValues(co);
							...
							if (i == 0) {
								setNativeObjectValue((NativeObject) outCO
										.getHasNativeObjects().get(0),
										omElement.get(i));
							} else {
								NativeObject co = ((NativeObject) outCO
										.getHasNativeObjects().get(0))
										.cloneTheNO();
		...
		try {
			if (co.getHasNativeObjects() != null
				...
				NativeObject no1 = (NativeObject) co.getHasNativeObjects().get(
						0);
				Vector<OMElementImpl> vec = findTheArrayObjectNodesInOMElement(
						omElement, no1, null);
				for (int i = 0; i < vec.size(); i++) {
					if (i == 0) {
			...
			} else if (co.getHasComplexObjects() != null
					&& co.getHasComplexObjects().size() > 0) {
	...
	private static OMElementImpl findTheComplexObjectNodeInOMElement(
			OMElementImpl omElement, ComplexObject co) {
	...
	private static Vector<OMElementImpl> findTheArrayObjectNodesInTheResponseBody(
	...
	private static Vector<OMElementImpl> findTheArrayObjectNodesInOMElement(
			OMElementImpl omElement, NativeObject no, ComplexObject co) {
		...
		if (vec.size() == 0) {
			Iterator iter2 = omElement.getChildrenWithName(new QName(
					objectQName.getNamespaceURI(), "item"));
			while (iter2.hasNext()) {
				org.apache.axiom.om.impl.llom.OMElementImpl childOMElement = (OMElementImpl) iter2
						.next();
				vec.add((OMElementImpl) childOMElement);
			}
		}
