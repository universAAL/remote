...
public class Axis2_RpcEncodedMessageBuilder {
    ...
    public static SOAPEnvelope createSOAPEnvelope_RPC_Encoded(QName operationName, WSOperationInput operationInput, ParsedWSDLDefinition theDefinition){
...
//        envelope.getHeader().addHeaderBlock(arg0, arg1)
    ...
    private static OMElement createOMElementForNativeObjectInput(SOAPFactory fac, NativeObject no, OMNamespace operationNs){
        //OMElement valueA = fac.createOMElement(no.getObjectName(), operationNamespace);
        //if(no.isOptional&&(no.hasValue==null||no.hasValue.length()==0))return null;
        if((no.getHasValue()==null||no.getHasValue().length()==0))return null;
        ...
        //valueA.addAttribute("type", no.getObjectType().getNamespaceURI()+":"+no.getObjectType().getLocalPart(), schemaInstanceNs);
        ...
        if(no.getObjectType().getPrefix()!=null&&no.getObjectType().getPrefix().length()>0){
            ...
            valueA.addAttribute("type", no.getObjectType().getPrefix()+":"+no.getObjectType().getLocalPart(), schemaInstanceNs);
        }else{
    ...
    private static OMElement createOMElementForComplexObjectInput(SOAPFactory fac, ComplexObject co, OMNamespace operationNamespace){
        System.out.println();
        if(!Axis2WebServiceInvoker.theComplexObjectCarriesValues_ITERATIVE(co))return null;
            ...
            ComplexObject co1=(ComplexObject)iter2.next();
             ...
             if(co1.isIsArrayType()){
                ...
                if(co1.getHasComplexObjects().size()>0){
                    ...
                    Vector v=createOMElementForArrayTypeObject_ContainingNativeObjs_Input(fac, co1, operationNamespace);
                    if(v!=null){
                        Iterator iter111=v.iterator();
                        while(iter111.hasNext()){
                            OMElement valueForNO111 = (OMElement)iter111.next();
    ...
    private static OMElement createOMElementForArrayTypeObjectInput(SOAPFactory fac, ComplexObject co, OMNamespace operationNamespace){
    ...
    private static Vector createOMElementForArrayTypeObject_ContainingNativeObjs_Input(SOAPFactory fac, ComplexObject co, OMNamespace operationNamespace){
        //OMElement valueA = fac.createOMElement(co.getObjectName(), operationNamespace);
