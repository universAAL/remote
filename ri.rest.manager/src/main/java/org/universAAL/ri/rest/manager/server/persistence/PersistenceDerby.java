...
public class PersistenceDerby implements Persistence {
	...
	if (checkIdExists(s.getId(),T_SPACES)) {
	    storeREGISTERS = "update " + T_SPACES + " SET callback='"+ s.getCallback() + "', tstmp='" + t +"'"+ (v!=null?", version='"+v+"'":"") +" WHERE id='" + s.getId() + "'";
	}else{
	    storeREGISTERS = "insert into " + T_SPACES + " (id, callback, tstmp, version) values ('" + s.getId() + "','" + s.getCallback() + "','" + t + "', " + (v!=null?"'"+v+"'":"NULL") +")";
	}
	...
	if (checkSubIdExists(space, s.getId(), T_SUBSCRIBERS)) {
	    exec = "update " + T_SUBSCRIBERS + " SET serial='" + s.getPattern() + "', callback='"+s.getCallback()+"', tstmp='"+t+"' WHERE id='"+space+"' AND subid='"+s.getId()+"'";
	}else{
	    exec = "insert into " + T_SUBSCRIBERS+ " (id, subid, serial, callback, tstmp) values ('" + space + "','"+ s.getId() + "','" + s.getPattern() + "','"+ s.getCallback() + "','"+t+"')";
	}
	executeGeneric(exec);
    }
	...
	if (checkSubIdExists(space, p.getId(), T_PUBLISHERS)) {
	    exec = "update " + T_PUBLISHERS + " SET serial='" + p.getProviderinfo() + "', tstmp='"+t+"' WHERE id='"+space+"' AND subid='"+p.getId()+"'";
	}else{
	    exec = "insert into " + T_PUBLISHERS+ " (id, subid, serial, tstmp) values ('" + space + "','"+ p.getId() + "','" + p.getProviderinfo() + "','"+t+"')";
	}
	executeGeneric(exec);
    }
	...
	if (checkSubIdExists(space, c.getId(), T_CALLEES)) {
	    exec = "update " + T_CALLEES + " SET serial='" + c.getProfile() + "', callback='"+c.getCallback()+"', tstmp='"+t+"' WHERE id='"+space+"' AND subid='"+c.getId()+"'";
	}else{
	    exec = "insert into " + T_CALLEES+ " (id, subid, serial, callback, tstmp) values ('" + space + "','"+ c.getId() + "','" + c.getProfile() + "','" + c.getCallback() + "','"+t+"')";
	}
	executeGeneric(exec);
    }
	...
	try {
	    ...
	    // Again, put individual try/catch to keep deleting if errors. I dont trust addBatch.
	    try {
		stmt.executeUpdate(execCee);
	    } catch (SQLException e) {
		if (!e.getSQLState().equals("X0Y32")) {
	    ...
	    try {
		stmt.executeUpdate(execSpace);// TODO Remove in cascade? Rsult is the same, but maybe faster?
	    } catch (SQLException e) {
		if (!e.getSQLState().equals("X0Y32")) {
		    e.printStackTrace();
		}
	...
	try {
		...
		if(id!=null && subid!=null && serial!=null){
		    ...
		    UaalWrapper.getInstance().getTenant(id).addContextSubscriber(new SubscriberWrapper(Activator.getUaalContext(), new ContextEventPattern[]{cep}, new Subscriber(id, subid, callback, serial), id));
		}
	    }
		...
		if(id!=null && subid!=null && serial!=null){
		    ...
		    UaalWrapper.getInstance().getTenant(id).addServiceCallee(new CalleeWrapper(Activator.getUaalContext(), new ServiceProfile[]{sp}, new Callee(id, subid, callback, serial), id));
		}
	    }
		...
		if(id!=null && subid!=null){
		    UaalWrapper.getInstance().getTenant(id).addServiceCaller(new CallerWrapper(Activator.getUaalContext(), new Caller(id, subid)));
		}
	    }
		...
		if(id!=null && subid!=null && serial!=null){
		    ...
		    UaalWrapper.getInstance().getTenant(id).addContextPublisher(new PublisherWrapper(Activator.getUaalContext(), cp, new Publisher(id, subid, serial)));
		}
	    }
	...
	try {
	    ...
	    String execCee = "delete from " + T_CALLEES + " WHERE {fn TIMESTAMPDIFF( SQL_TSI_DAY, tstmp, '"
		    + t.toString() + "')} > 0";
	    String execSub = "delete from "
		    + T_SUBSCRIBERS+ " WHERE {fn TIMESTAMPDIFF( SQL_TSI_DAY, tstmp, '"
		    + t.toString() + "')} > 0";
	    String execCer = "delete from " + T_CALLERS + " WHERE {fn TIMESTAMPDIFF( SQL_TSI_DAY, tstmp, '"
		    + t.toString() + "')} > 0";
	    String execPub = "delete from "
		    + T_PUBLISHERS+ " WHERE {fn TIMESTAMPDIFF( SQL_TSI_DAY, tstmp, '"
		    + t.toString() + "')} > 0";
	    String execSpace = "delete from "
		    + T_SPACES + " WHERE {fn TIMESTAMPDIFF( SQL_TSI_DAY, tstmp, '"
		    + t.toString() + "')} > 0";
