...
public class Axis1ParserWrapper {
        ...
        try {
            ...
            try {
                if (axis1Parser.getCurrentDefinition().getServices() != null) {
                    Collection servicesMap = axis1Parser.getCurrentDefinition().getServices().values();
                    Iterator iter1 = servicesMap.iterator();
                    while (iter1.hasNext()) {
                        com.ibm.wsdl.ServiceImpl serv = (com.ibm.wsdl.ServiceImpl) iter1.next();
                        ...
                        while (portsIter.hasNext()) {
                            try {
                                com.ibm.wsdl.PortImpl portImpl = (com.ibm.wsdl.PortImpl) portsIter.next();
                                Iterator extElemsIter = portImpl.getExtensibilityElements().iterator();
                                while (extElemsIter.hasNext()) {
                                    com.ibm.wsdl.extensions.soap.SOAPAddressImpl sai = (com.ibm.wsdl.extensions.soap.SOAPAddressImpl) extElemsIter.next();
                                    theParsedDefinition.setServiceURL(sai.getLocationURI());
                                    soapPortFound = true;
                                }
                            } catch (Exception e) {
                                //e.printStackTrace();
                        ...
                        if (!soapPortFound) {
                            theParsedDefinition.getContainingErrors().add("ERROR! the service '" + serv.getQName().getLocalPart() + "' did not contain SOAP PORT!!!  (contains " + portsMap.size() + " ports!  ....!=1)");
                        }
                    }
                }
            } catch (Exception e) {
            ...
            theParsedDefinition.setTargetNamespaceURI(theAxisDefinition.getTargetNamespace());
            parseTheWSDLOperations(theSymbolTable, theAxisDefinition, theParsedDefinition);
    ...
    private static void parseComplexElement(DefinedElement definedElement, SymbolTable theSymbolTable, ComplexObject co,
        ...
        if (definedElement.getContainedElements() != null) {
            if (definedElement.getContainedElements().size() > 0) {
                Iterator containedElementsIterator = definedElement.getContainedElements().iterator();
                ...
                while (containedElementsIterator.hasNext()) {
                    ...
                    //Ta contained elements einai ta native i complex objects pou periexei to complex type
                    org.apache.axis.wsdl.symbolTable.ElementDecl containedElement =
                            (org.apache.axis.wsdl.symbolTable.ElementDecl) containedElementsIterator.next();
                    System.out.println();
                    System.out.println("\tcontainedElement isBaseType: " + containedElement.getType().isBaseType());

                    if (containedElement.getMaxOccursIsUnbounded() && definedElement.getContainedElements().size() == 1) {
                        //Array Type
                        ...
                        if (containedElement.getType() != null && containedElement.getType().getRefType() != null) {
                            System.out.println(containedElement.getType().getQName().getLocalPart());//Type tou CO
                            QName objectTypeQName = containedElement.getType().getQName();
                            if (containedElement.getType().getQName().getLocalPart() != null &&
                                    containedElement.getType().getQName().getLocalPart().contains("[")) {
                            } else {
                                objectTypeQName = new QName(containedElement.getType().getQName().getNamespaceURI(),
                                        containedElement.getType().getQName().getLocalPart() + "[]");
                            }
                            System.out.println(containedElement.getType().getRefType().isBaseType());//apo dw tha katalavw an einai nativeObject i Complex
                            //co.setAdditionalInfo(containedElement.getType().getRefType().getQName().getNamespaceURI());//Namespace tou CO
                            ...
                            if (containedElement.getType().getRefType().isBaseType()) {
                                ...
                                parseComplexType(containedElementType, theSymbolTable, co, theParsedDefinition);
                                System.out.println();
                            }

                        } else {
                            theParsedDefinition.getContainingErrors().add("ERROR 1234 @line ~150");
                        ...
                        //Gia kathe containedElement ftiaxnw kainourgio ComplexObject i NO kai to vazw mesa sto CO
                        if (containedElement.getType() != null) {
                            if (containedElement.getType().isBaseType()) {
                                ...
                                no1.setObjectName(new QName(containedElement.getQName().getNamespaceURI(),
                                        containedElement.getQName().getLocalPart().substring(containedElement.getQName().getLocalPart().indexOf(">") + 1),
                                        containedElement.getQName().getPrefix()));

                                System.out.println(containedElement.getType().getQName().getLocalPart());//Type tou CO1
                                //no1.setObjectType(containedElement.getType().getQName().getLocalPart());
                                System.out.println(containedElement.getType().isBaseType());//apo dw tha katalavw an einai nativeObject i Complex
                                //no1.setNamespaceURI(containedElement.getType().getQName().getNamespaceURI());
                                no1.setObjectType(containedElement.getType().getQName());
                                //no1.setAdditionalInfo(containedElement.getType().getQName().getNamespaceURI());//Namespace tou CO1                            

                                //DefinedType containedElementType=(DefinedType)containedElement.getType();
                                //parseComplexType(containedElementType, theSymbolTable, co1);
                                ...
                                //co1.setAdditionalInfo(containedElement.getType().getQName().getNamespaceURI());//Namespace tou CO1
                            ...
                            theParsedDefinition.getContainingErrors().add("ERROR 1234 @line ~193");
                ...
                if (definedElement.getRefType() != null) {
                    System.out.println(definedElement.getRefType().getQName().getLocalPart());//Type tou CO
                    //co.setObjectType(definedElement.getRefType().getQName().getLocalPart()+"[]");
                    System.out.println(definedElement.getRefType().isBaseType());//apo dw tha katalavw an einai nativeObject i Complex
                    //co.setAdditionalInfo(definedType.getRefType().getQName().getNamespaceURI());//Namespace tou CO
                    //co.setNamespaceURI(definedElement.getRefType().getQName().getNamespaceURI());
                    co.setObjectType(new QName(definedElement.getRefType().getQName().getNamespaceURI(),
                            definedElement.getRefType().getQName().getLocalPart() + "[]",
                            definedElement.getRefType().getQName().getPrefix()));
                    ...
                    if (definedElement.getRefType().isBaseType()) {
                        ...
                        if (definedElement.getRefType().getClass().getName().contains("org.apache.axis.wsdl.symbolTable.DefinedType")) {
                            DefinedType containedElementType = (DefinedType) definedElement.getRefType();
                            parseComplexType(containedElementType, theSymbolTable, co, theParsedDefinition);
                            System.out.println();
                        } else if (definedElement.getRefType().getClass().getName().contains("org.apache.axis.wsdl.symbolTable.UndefinedType")) {
                            org.apache.axis.wsdl.symbolTable.UndefinedType containedElementType =
                                    (org.apache.axis.wsdl.symbolTable.UndefinedType) definedElement.getRefType();
                            System.out.println("WWW " + containedElementType.getName());
                            //parseComplexType(containedElementType, theSymbolTable, co);
                            System.out.println();
                        } else {
                            System.out.println("ERROR UnDefined TYPE...???? @line ~228");
                            theParsedDefinition.getContainingErrors().add("ERROR UnDefined TYPE...???? @line ~228");
                        }
                    }

                } else {
                    theParsedDefinition.getContainingErrors().add("ERROR 1234 @line ~233");
            ...
            } else if (definedElement.getRefType().getClass().getName().contains("DefinedType")) {
                System.out.println("WWW2");
                System.out.println("ERROR @line ~245");
                theParsedDefinition.getContainingErrors().add("ERROR 1234 @line ~245");
                //System.exit(-43);
            }
        }
    }

    private static void parseComplexType(DefinedType definedType, SymbolTable theSymbolTable, ComplexObject co,
            ParsedWSDLDefinition theParsedDefinition) {
        if (definedType == null) {
            return;
        }
        ...
        if (parsedComplexObjectsHashmap.containsKey(definedType.getQName())) {
            ComplexObject co1 = (ComplexObject) (parsedComplexObjectsHashmap.get(definedType.getQName()));
            /*
            if(co.getObjectName()!=null){
            QName coName=new QName(co.objectName.getNamespaceURI(),co.objectName.getLocalPart(),co.objectName.getPrefix());

            co=co1.cloneTheCO();
            co.setObjectName(coName);
            }else{
            co=co1.cloneTheCO();
            }*/
            ...
            co.getHasExtendedObjects().add(((ComplexObject)co1.getHasExtendedObjects().get(i)).cloneTheCO());
            }
            
            for(int i=0;i<co1.getHasComplexObjects().size();i++){
            co.getHasComplexObjects().add(((ComplexObject)co1.getHasComplexObjects().get(i)).cloneTheCO());
            }
            
            for(int i=0;i<co1.getHasNativeObjects().size();i++){
            co.getHasNativeObjects().add(((NativeObject)co1.getHasNativeObjects().get(i)).cloneTheNO());
            }*/
        ...
        if (set != null) {
            ...
            for (int ii = 0; ii < vec.size(); ii++) {
                org.apache.axis.wsdl.symbolTable.ElementDecl containedElement = (org.apache.axis.wsdl.symbolTable.ElementDecl) vec.get(ii);
                if (containedElement.getMaxOccursIsUnbounded() && definedType.getContainedElements().size() == 1) {
                    ...
                    if (containedElement.getType() != null) {
                        if (containedElement.getType().isBaseType()) {
                            ...
                            NativeObject no1 = new NativeObject();
                            no1.setObjectName(new QName(containedElement.getQName().getNamespaceURI(),
                                    containedElement.getQName().getLocalPart().substring(
                            ...
                            //no1.setNamespaceURI(containedElement.getType().getQName().getNamespaceURI());
                            no1.setObjectType(containedElement.getType().getQName());
                            if (containedElement.getMinOccursIs0() || containedElement.getNillable() || containedElement.getOptional()) {
                            ...
                            /*
                            if(definedType.getQName().getLocalPart().equals("GeoPlaceType")){
                            System.out.println("");
                            }*/
                            ...
                            if (n1.getChildNodes() != null) {
                                ...
                                for (int i = 0; i < n1.getChildNodes().getLength(); i++) {
                                    ...
                                    if (childNode.getNodeName() != null && childNode.getNodeName().contains("restriction")) {
                                        if (childNode.getChildNodes() != null) {
                                            ...
                                            for (int i1 = 0; i1 < childNode.getChildNodes().getLength(); i1++) {
                                                ...
                                                if (restrictionNode != null && restrictionNode.getNodeName() != null &&
                                                    ...
                                                    if (restrictionNode.getAttributes() != null && restrictionNode.getAttributes().getNamedItem("value") != null) {
                                                        System.out.println(restrictionNode.getAttributes().getNamedItem("value"));
                                                        no1.getHasAllowedValues().add(restrictionNode.getAttributes().getNamedItem("value"));
                                                    }
                                                }
                                            }
                                        }
        ...
        if (definedType.getContainedElements() != null) {
            if (definedType.getContainedElements().size() > 0) {
                Iterator containedElementsIterator = definedType.getContainedElements().iterator();
                //System.out.println("AAA"+definedType.getContainedElements().size());
                ...
                if (definedType.getRefType() != null) {
                    ...
                    System.out.println(definedType.getRefType().getQName().getLocalPart());//Type tou CO
                    co.setObjectType(new QName(definedType.getRefType().getQName().getNamespaceURI(),
                            definedType.getRefType().getQName().getLocalPart() + "[]",
                            definedType.getRefType().getQName().getPrefix()));
                    System.out.println(definedType.getRefType().isBaseType());//apo dw tha katalavw an einai nativeObject i Complex
                    //co.setAdditionalInfo(definedType.getRefType().getQName().getNamespaceURI());//Namespace tou CO
                    ...
                    if (definedType.getRefType().isBaseType()) {
                        ...
                        //ComplexObject
                        if (definedType.getRefType().getClass().getName().contains("org.apache.axis.wsdl.symbolTable.DefinedType")) {
                            ComplexObject co1 = new ComplexObject();
                            DefinedType containedElementType = (DefinedType) definedType.getRefType();
                            co1.setObjectType(containedElementType.getQName());
                            ...
                            org.apache.axis.wsdl.symbolTable.UndefinedType containedElementType =
                                    (org.apache.axis.wsdl.symbolTable.UndefinedType) definedType.getRefType();
                            System.out.println("WWW " + containedElementType.getName());
                            //parseComplexType(containedElementType, theSymbolTable, co);
                            System.out.println();
                        } else {
                            System.out.println("ERROR UnDefined TYPE...???? @line ~376");
                            theParsedDefinition.getContainingErrors().add("ERROR UnDefined TYPE...???? @line ~376");
                        }
                    }
                } else {
                    //theParsedDefinition.getContainingErrors().add("ERROR 123456!!! @line ~381");
                    Type t = theSymbolTable.getType(definedType.getQName());
                    if (t != null && t.getNode() != null && t.getNode().getAttributes() != null) {
                        Node isAbstractNode = t.getNode().getAttributes().getNamedItem("abstract");
                        if (isAbstractNode != null && isAbstractNode.getNodeValue() != null && isAbstractNode.getNodeValue().equalsIgnoreCase("true")) {
    ...
    private static void parseSimpleType(Type t, SymbolTable theSymbolTable, NativeObject no) {
        no.setObjectType(t.getQName());
        //no.setNamespaceURI(t.getQName().getNamespaceURI());
    }

    private static void parseTheWSDLOperations(SymbolTable theSymbolTable, Definition theAxisDefinition, ParsedWSDLDefinition theParsedDefinition) {
        ...
        while (portTypesIterator.hasNext()) {
            ...
            System.out.println("Port Type name: " + portType.getQName().toString());//+"     class: "+p.getBinding().getPortType().getQName().toString());//+"   binding:"+p.getBinding());

            if (portType.getQName().getLocalPart().contains("Http") || portType.getQName().getLocalPart().contains("HTTP")) {
                continue;
            }
            ...
            if (operationsList != null) {
                ...
                while (operationsIter.hasNext()) {
                    OperationImpl operation = (OperationImpl) operationsIter.next();
                    ...
                    while (bindingsIter.hasNext()) {
                        com.ibm.wsdl.BindingImpl binding = (com.ibm.wsdl.BindingImpl) bindingsIter.next();

                        if (binding.getQName().getLocalPart().contains("Http") || binding.getQName().getLocalPart().contains("HTTP")) {
                            continue;
                        }

                        List bindingOperations = binding.getBindingOperations();
                        Iterator bindingOperationsIter = bindingOperations.iterator();
                        while (bindingOperationsIter.hasNext()) {
                            com.ibm.wsdl.BindingOperationImpl bindingOperation = (com.ibm.wsdl.BindingOperationImpl) bindingOperationsIter.next();
                            if (bindingOperation != null && bindingOperation.getOperation() != null &&
                                    bindingOperation.getOperation().equals(operation)) {
                                List extElementsList = bindingOperation.getExtensibilityElements();
                                Iterator extElementsIter = extElementsList.iterator();
                                while (extElementsIter.hasNext()) {
                                    Object obj = extElementsIter.next();
                                    if (obj != null && obj.getClass().getName().contains("SOAPOperationImpl")) {
                                        try {
                                            com.ibm.wsdl.extensions.soap.SOAPOperationImpl extElement =
                                                    (com.ibm.wsdl.extensions.soap.SOAPOperationImpl) obj;
                                            mitsosOperation.setHasBindingSoapAction(extElement.getSoapActionURI());
                                            //System.out.println(extElement.getElementType());
                                            mitsosOperation.setHasStyle(extElement.getStyle());
                                        } catch (Exception e) {
                                            e.printStackTrace();
                                        }
                                    } else if (obj != null && obj.getClass().getName().contains("SOAPOperationImpl")) {
                                        try {
                                            com.ibm.wsdl.extensions.soap12.SOAP12OperationImpl extElement =
                                                    (com.ibm.wsdl.extensions.soap12.SOAP12OperationImpl) obj;
                                            mitsosOperation.setHasBindingSoapAction(extElement.getSoapActionURI());
                                            //System.out.println(extElement.getElementType());
                                            mitsosOperation.setHasStyle(extElement.getStyle());
                                        } catch (Exception e) {
                                ...
                                com.ibm.wsdl.BindingInputImpl bindingInput = (com.ibm.wsdl.BindingInputImpl) bindingOperation.getBindingInput();
                                List extElemsList = bindingInput.getExtensibilityElements();
                                ...
                                while (iter.hasNext()) {
                                    ...
                                    if (obj instanceof com.ibm.wsdl.extensions.soap.SOAPHeaderImpl) {
                                        ...
                                        //to message exei mesa to type pou prepei na vrw edw kai na valw sto ComplexObject tou Header
                                        //QName partQName=new QName(sh.getPart(), sh.getMessage().getNamespaceURI());

                                        com.ibm.wsdl.MessageImpl msg = (com.ibm.wsdl.MessageImpl) theAxisDefinition.getMessage(sh.getMessage());
                                        if (msg.getParts() != null && msg.getParts().values() != null) {
                                            Iterator iter1 = msg.getParts().values().iterator();
                                            while (iter1.hasNext()) {
                                                com.ibm.wsdl.PartImpl partImpl = (com.ibm.wsdl.PartImpl) iter1.next();

                                                Type typeGotFromTheSymbolTable = theSymbolTable.getType(partImpl.getTypeName());
                                                if (typeGotFromTheSymbolTable != null) {

                                                    Object nativeOrComplexObject = parseTypeGotFromTheSymbolTable(typeGotFromTheSymbolTable, theSymbolTable,
                                                            sh.getMessage(), theParsedDefinition);
                                                    if (nativeOrComplexObject != null) {
                                                        ...
                                                        } else if (nativeOrComplexObject.getClass().getName().contains("ComplexObject")) {
                                                    ...
                                                    Element elementGotFromTheSymbolTable = theSymbolTable.getElement(sh.getMessage());
                                                    if (elementGotFromTheSymbolTable != null) {
                                                        Object nativeOrComplexObject = parseElementGotFromTheSymbolTable(elementGotFromTheSymbolTable, theSymbolTable,
                                                                sh.getMessage(), theParsedDefinition);
                                                        if (nativeOrComplexObject != null) {
                                                            if (nativeOrComplexObject.getClass().getName().contains("NativeObject")) {
                                                                NativeObject no = (NativeObject) nativeOrComplexObject;
                                                        ...
                                                        theParsedDefinition.getContainingErrors().add("ERROR @line ~589!!!");
                                                    }
                                                }
                                ...
                                System.out.println(bindingOperation.getBindingInput().getClass().getName());
                            }
                        }
                    ...
                    OutputImpl operationOutput = (OutputImpl) operation.getOutput();

                    System.out.println(operationInput.getMessage().getQName().toString() + "    " + operationInput.getMessage().getParts().size());
                    System.out.println(operationInput.getMessage().getClass().getName());
                    System.out.println("\t" + operationInput.getMessage().getParts().toString());

                    MessageImpl inputMessageOfOperation = (MessageImpl) operationInput.getMessage();

                    if (inputMessageOfOperation.getParts() != null && inputMessageOfOperation.getParts().size() > 0) {
                        Map inputPartsMap = inputMessageOfOperation.getParts();
                        Collection inputPartsCol = inputPartsMap.values();
                        if (inputPartsCol != null) {
                            Iterator inputPartsIterator = inputPartsCol.iterator();
                            while (inputPartsIterator.hasNext()) {
                                com.ibm.wsdl.PartImpl inputPart = (com.ibm.wsdl.PartImpl) inputPartsIterator.next();

                                if (inputPart.getTypeName() != null) {
                                    Type typeGotFromTheSymbolTable = theSymbolTable.getType(inputPart.getTypeName());

                                    if (typeGotFromTheSymbolTable != null) {
                                        Object nativeOrComplexObject = null;
                                        if (inputPart.getElementName() != null) {
                                            nativeOrComplexObject = parseTypeGotFromTheSymbolTable(typeGotFromTheSymbolTable, theSymbolTable,
                                                    inputPart.getElementName(), theParsedDefinition);
                                        } else if (inputPart.getName() != null) {
                                            nativeOrComplexObject = parseTypeGotFromTheSymbolTable(typeGotFromTheSymbolTable, theSymbolTable,
                                                    new QName(inputPart.getName()), theParsedDefinition);
                                        } else {
                                            nativeOrComplexObject = parseTypeGotFromTheSymbolTable(typeGotFromTheSymbolTable, theSymbolTable,
                                                    inputPart.getTypeName(), theParsedDefinition);
                                        }
                                        if (nativeOrComplexObject != null) {
                                            mitsosOperationInput.getHasNativeOrComplexObjects().add(nativeOrComplexObject);
                                        }

                                    } else {
                                        /*
                                        Element elem = theSymbolTable.getElement(inputPart.getElementName());
                                        System.out.println(elem.getQName().toString());
                                        QName elemName=inputPart.getElementName();
                                         */
                                        System.out.println("ERROR @line ~283!!!");
                                        theParsedDefinition.getContainingErrors().add("ERROR @line ~447!!!");
                                        //document literal???
                                    }

                                } else if (inputPart.getElementName() != null) {
                                    //LITERAL TYPE!!!
                                    Type typeGotFromTheSymbolTable = theSymbolTable.getType(inputPart.getElementName());
                                    ...
                                    if (typeGotFromTheSymbolTable != null) {
                                        ...
                                        Element elementGotFromTheSymbolTable = theSymbolTable.getElement(inputPart.getElementName());
                                        if (elementGotFromTheSymbolTable != null) {
                                            Object nativeOrComplexObject = parseElementGotFromTheSymbolTable(elementGotFromTheSymbolTable, theSymbolTable,
                                                    inputPart.getElementName(), theParsedDefinition);
                                            if (nativeOrComplexObject != null) {
                                            ...
                                            theParsedDefinition.getContainingErrors().add("ERROR @line ~471!!!");
                                        }
                                    }
                                }
                    ...
                    MessageImpl outputMessageOfOperation = (MessageImpl) operationOutput.getMessage();
                    WSOperationOutput mitsosOperationOutput = new WSOperationOutput();

                    if (outputMessageOfOperation.getParts() != null && outputMessageOfOperation.getParts().size() > 0) {
                        Map outputPartsMap = outputMessageOfOperation.getParts();
                        Collection outputPartsCol = outputPartsMap.values();
                        if (outputPartsCol != null) {
                            Iterator outputPartsIterator = outputPartsCol.iterator();
                            while (outputPartsIterator.hasNext()) {
                                com.ibm.wsdl.PartImpl outputPart = (com.ibm.wsdl.PartImpl) outputPartsIterator.next();
                                //System.out.println("\t\t"+outputPart.getName()+"   "+outputPart.getTypeName().toString());  

                                if (outputPart.getTypeName() != null) {
                                    Type typeGotFromTheSymbolTable = theSymbolTable.getType(outputPart.getTypeName());
                                    ...
                                    if (typeGotFromTheSymbolTable != null) {
                                        ...
                                        if (outputPart.getElementName() != null) {
                                            nativeOrComplexObject = parseTypeGotFromTheSymbolTable(typeGotFromTheSymbolTable, theSymbolTable,
                                                    outputPart.getElementName(), theParsedDefinition);
                                        } else if (outputPart.getName() != null) {
                                            nativeOrComplexObject = parseTypeGotFromTheSymbolTable(typeGotFromTheSymbolTable, theSymbolTable,
                                                    new QName(outputPart.getName()), theParsedDefinition);
                                        } else {
                                            nativeOrComplexObject = parseTypeGotFromTheSymbolTable(typeGotFromTheSymbolTable, theSymbolTable,
                                                    outputPart.getTypeName(), theParsedDefinition);
                                        }

                                        if (nativeOrComplexObject != null) {
                                            mitsosOperationOutput.getHasNativeOrComplexObjects().add(nativeOrComplexObject);
                                        }
                                    } else {
                                        /*
                                        Element elem = theSymbolTable.getElement(inputPart.getElementName());
                                        ...
                                        theParsedDefinition.getContainingErrors().add("ERROR @line ~514!!!");
                                        //document literal???
                                    }
                                } else if (outputPart.getElementName() != null) {
                                    //LITERAL TYPE!!!
                                    Type typeGotFromTheSymbolTable = theSymbolTable.getType(outputPart.getElementName());
                                    ...
                                    if (typeGotFromTheSymbolTable != null) {
                                        ...
                                        Element elementGotFromTheSymbolTable = theSymbolTable.getElement(outputPart.getElementName());
                                        if (elementGotFromTheSymbolTable != null) {
                                            Object nativeOrComplexObject = parseElementGotFromTheSymbolTable(elementGotFromTheSymbolTable, theSymbolTable,
                                                    outputPart.getElementName(), theParsedDefinition);
                                            if (nativeOrComplexObject != null) {
                                            ...
                                            theParsedDefinition.getContainingErrors().add("ERROR @line ~538!!!");
                                        }
                                        //document literal???
                                    }
                                }
                    ...
                    theParsedDefinition.getWsdlOperations().add(mitsosOperation);
                }
            }
        }
    ...
    private static Object parseElementGotFromTheSymbolTable(Element elementGotFromTheSymbolTable, SymbolTable theSymbolTable, QName objectName,
            ParsedWSDLDefinition theParsedDefinition) {
        if (elementGotFromTheSymbolTable == null) {
            return null;
        }
        System.out.println(elementGotFromTheSymbolTable.getClass().getName());
        if (elementGotFromTheSymbolTable.getClass().getName().contains("org.apache.axis.wsdl.symbolTable.DefinedElement")) {
            org.apache.axis.wsdl.symbolTable.DefinedElement definedElement =
                    (org.apache.axis.wsdl.symbolTable.DefinedElement) elementGotFromTheSymbolTable;
            //ComplexObject
            ...
            parseComplexElement(definedElement, theSymbolTable, co, theParsedDefinition);

            return co;

        } else {
            theParsedDefinition.getContainingErrors().add("ERROR @ line ~ 577!!!");
    ...
    private static Object parseTypeGotFromTheSymbolTable(Type typeGotFromTheSymbolTable, SymbolTable theSymbolTable, QName objectName,
            ParsedWSDLDefinition theParsedDefinition) {
        if (typeGotFromTheSymbolTable == null) {
            return null;
        }
        ...
        System.out.println("\t" + typeGotFromTheSymbolTable.getClass().getName());
        if (typeGotFromTheSymbolTable.getClass().getName().contains("org.apache.axis.wsdl.symbolTable.BaseType")) {
            org.apache.axis.wsdl.symbolTable.BaseType nativeType = (org.apache.axis.wsdl.symbolTable.BaseType) typeGotFromTheSymbolTable;
            if (nativeType.getQName() != null && nativeType.getQName().getLocalPart().equals("Vector")) {
                //WW VECTOR!!!
                ComplexObject co = new ComplexObject();
                co.setObjectName(objectName);
        ...
        } else if (typeGotFromTheSymbolTable.getClass().getName().contains("org.apache.axis.wsdl.symbolTable.DefinedType")) {
            //ComplexObject
            ComplexObject co = new ComplexObject();
            co.setObjectName(objectName);
            ...
            //co.setAdditionalInfo(definedType.getQName().getNamespaceURI());
            parseComplexType(definedType, theSymbolTable, co, theParsedDefinition);
            ...
            theParsedDefinition.getContainingErrors().add("ERROR @ line ~ 617!!!");
            //System.exit(-13);            
        }
        return null;
    }
