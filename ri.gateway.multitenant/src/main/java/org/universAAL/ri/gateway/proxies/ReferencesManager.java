...
public class ReferencesManager {
	...
	private void put(final String scope, final BusMemberReference remoteReference) {
	...
	public synchronized void removeRemoteProxyReference(final BusMemberReference remoteReference) {
		final Set<BusMemberReference> existing = references.get(remoteReference.getChannel().getScope());
		if (existing != null) {
	...
	public Collection<BusMemberReference> getReferencesFor(final List<String> scopes) {
		final Set<BusMemberReference> all = new HashSet<BusMemberReference>();
