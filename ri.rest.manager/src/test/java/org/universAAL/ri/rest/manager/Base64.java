...
public final class Base64 {
    ...
    public static byte[] encodeBytesToBytes(byte[] source, int off, int len,
        ...
        if (off + len > source.length) {
            throw new IllegalArgumentException("Cannot have offset of " + off
                    + " and length of " + len + " with array of length "
                    + source.length);
        } // end if: off < 0
    ...
    private static int decode4to3(byte[] source, int srcOffset,
        ...
        if (srcOffset < 0 || srcOffset + 3 >= source.length) {
            throw new IllegalArgumentException("Source array with length "
                    + source.length + " cannot have offset of " + srcOffset
                    + " and still process four bytes.");
        } // end if
        if (destOffset < 0 || destOffset + 2 >= destination.length) {
            throw new IllegalArgumentException("Destination array with length "
                    + destination.length + " cannot have offset of "
                    + destOffset + " and still store three bytes.");
        } // end if
    ...
    public static byte[] decode(byte[] source, int off, int len, int options)
        ...
        if (off < 0 || off + len > source.length) {
            throw new IllegalArgumentException("Source array with length "
                    + source.length + " cannot have offset of " + off
                    + " and process " + len + " bytes.");
        } // end if
        ...
        for (i = off; i < off + len; i++) { // Loop through source
            ...
            if (sbiDecode >= WHITE_SPACE_ENC) {
                ...
                throw new java.io.IOException(
                        "Bad Base64 input character decimal " + cd
                                + " in array position " + i);
            } // end else:
