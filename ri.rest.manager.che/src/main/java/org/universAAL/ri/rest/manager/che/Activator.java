 ...
 */
package org.universAAL.ri.rest.manager.che;
...
import org.universAAL.ri.rest.manager.che.resources.CHe;
...
public class Activator implements BundleActivator {
	...
	public void start(BundleContext bcontext) throws Exception {
		...
		Activator.mContext = OSGiContainer.THE_CONTAINER
		...
		String filter2 = "(objectclass="
				+ MessageContentSerializerEx.class.getName() + ")";
		osgiContext.addServiceListener(serializerListener, filter2);
		referencesSerializer = osgiContext.getServiceReferences((String) null,
				filter2);
		for (int i = 0; referencesSerializer != null
				&& i < referencesSerializer.length; i++) {
			serializerListener.serviceChanged(new ServiceEvent(
					ServiceEvent.REGISTERED, referencesSerializer[i]));
		}

		// Instance persistence DB and before it is public in servlet
		try {
			persistence = (Persistence) Class
					.forName(Configuration.getDBClass())
		...
		sf.setResourceClasses(CHe.class);
		sf.setProvider(new Authenticator());
		sf.setResourceProvider(CHe.class, new SingletonResourceProvider(
				new CHe()));
	...
	public void stop(BundleContext arg0) throws Exception {
		for (int i = 0; tenantRefs != null && i < tenantRefs.length; i++) {
			tenantListener.serviceChanged(new ServiceEvent(
					ServiceEvent.UNREGISTERING, tenantRefs[i]));
		}
	...
	private class SerializerListener implements ServiceListener {
		public void serviceChanged(ServiceEvent event) {
			...
			case ServiceEvent.REGISTERED:
			case ServiceEvent.MODIFIED:
				parser = (MessageContentSerializerEx) osgiContext
