...
public class DocumentStyleWSDLParser {
    ...
    private static ComplexObject getExtendedTypeAsAComplexObject(Node theBaseNodeOfTheExtendedType, Schema theWSDLSchema){
        
        if(theBaseNodeOfTheExtendedType==null||theWSDLSchema==null)return null;
        ...
        if(theBaseNodeOfTheExtendedType.getAttributes().getNamedItem("name")!=null){
            ...
            theComplexObjectRepresentingTheComplexType.setObjectName(new QName(theBaseNodeOfTheExtendedType.getAttributes().getNamedItem("name").getNodeValue()));
            theComplexObjectRepresentingTheComplexType.setObjectType(new QName(theBaseNodeOfTheExtendedType.getAttributes().getNamedItem("name").getNodeValue()));
            
            boolean objectExistsInHashmap=MitsosParser.parsedObjectsHashmap.containsKey(theBaseNodeOfTheExtendedType.getAttributes().getNamedItem("name").getNodeValue());            
            if(objectExistsInHashmap){
                return (ComplexObject)MitsosParser.parsedObjectsHashmap.get(theComplexObjectRepresentingTheComplexType.getObjectType());
            }
            MitsosParser.parsedObjectsHashmap.put(theComplexObjectRepresentingTheComplexType.getObjectType(), theComplexObjectRepresentingTheComplexType);
        ...
        if(listWithChildrenOfTheComplexTypeBaseNode!=null){
            for(int k=0;k<listWithChildrenOfTheComplexTypeBaseNode.getLength();k++){
                ...
                if(listWithChildrenOfTheComplexTypeBaseNode.item(k).getNodeName().contains("sequence")||
                    ...
                    parseSequenceOrChoiceNode(theWSDLSchema, listWithChildrenOfTheComplexTypeBaseNode.item(k), 0, theComplexObjectRepresentingTheComplexType);                 

                }else if(listWithChildrenOfTheComplexTypeBaseNode.item(k).getNodeName().contains("complexContent")){
                    parseComplexContentForDocumentType(theWSDLSchema,listWithChildrenOfTheComplexTypeBaseNode.item(k), 0, theComplexObjectRepresentingTheComplexType);
                }
            }
        }
    ...
    private static void addTypesOfExtendedTypeToTheComplexObject(ComplexObject co, String extendedTypeName, 
        ...
        Node theBaseNodeOfTheExtendedType=WSDL_XSD_SchemaToolkit.getNodeOfType(extendedTypeName, theWSDLxsdSchema, null);
        if(theBaseNodeOfTheExtendedType!=null){
        //-System.out.println(theBaseNodeOfTheExtendedType.getLocalName());
            ComplexObject extendedTypeCO=getExtendedTypeAsAComplexObject(theBaseNodeOfTheExtendedType, theWSDLxsdSchema);
            if(extendedTypeCO!=null){
                //The content of the extendedTypeCO should become content of the co                
    ...
    private static void parseSimpleContentForDocumentType(Schema theWSDLxsdSchema, Node inputNode, int iterNumber, ComplexObject co) {
        ...
        //Mesa stous nodes tis listas childrenOfTheChildrenOfTheDamned tha yparxoun oi nodes "extension" (logika tha yparxei moono enas tetoios node)
        //Mesa stous nodes "extension" tha yparxoun oi nodes element pou tha einai ta simple object i complex object pou symplirwnoun ta klironomimena
         if(childrenOfTheChildrenOfTheDamned!=null){
            ...
            for(int k2=0;k2<childrenOfTheChildrenOfTheDamned.getLength();k2++){
               // ////-System.out.println("FFF "+childrenOfTheChildrenOfTheDamned.item(k2).getNodeName());
                if(!childrenOfTheChildrenOfTheDamned.item(k2).hasAttributes())continue;
                ...
                if(childrenOfTheChildrenOfTheDamned.item(k2).getNodeName().contains("extension")){
                    if(childrenOfTheChildrenOfTheDamned.item(k2).getAttributes()!=null){
                        if(childrenOfTheChildrenOfTheDamned.item(k2).getAttributes().getNamedItem("base")!=null){
                            String baseType=childrenOfTheChildrenOfTheDamned.item(k2).getAttributes().getNamedItem("base").getNodeValue();                            
                            co.setObjectType(new QName(childrenOfTheChildrenOfTheDamned.item(k2).getAttributes().getNamedItem("base").getNodeValue()));
                        }
                    }
                }
            }
    ...
    private static void parseComplexContentForDocumentType(Schema theWSDLxsdSchema, Node inputNode, int iterNumber, ComplexObject co) {
        //O inputNode einai aytos pou exei name "complexContent"        
        //-System.out.println("*** parseComplexContentForDocumentType ***");
        NodeList childrenOfTheChildrenOfTheDamned=inputNode.getChildNodes();
        //Mesa stous nodes tis listas childrenOfTheChildrenOfTheDamned tha yparxoun oi nodes "extension" (logika tha yparxei moono enas tetoios node)
                ...
                if(childrenOfTheChildrenOfTheDamned.item(k2).getNodeName().contains("extension")){
                    if(childrenOfTheChildrenOfTheDamned.item(k2).getAttributes()!=null){
                        if(childrenOfTheChildrenOfTheDamned.item(k2).getAttributes().getNamedItem("base")!=null){
                            ...
                            /*
                            if(baseType.contains(":")){//DEN Ginetai na einai simple type to base tou extended type,
                                //ara vlepw mono ayti tin periptwsi...
                                
                                String key1=baseType.substring(0, baseType.indexOf(":"));
                                String xsdNamespace=(String)namespaces.get(key1);
                                if(xsdNamespace!=null){
                                 //   ////-System.out.println("Namespace Found!!!");   
                                    String type1=baseType.substring(key1.length()+1, baseType.length());
                                    
                                    parseImportedXSDforLiteral3(s1, type1, ta, iterNumber+1,true,co,
                                            xsdNamespace);
                    ...
                    //parse children "elements" of the extension node - "name"=sequence i choice
                    ...
                    for(int i=0;i<elementTypesOfExtendedTypeList.getLength();i++){
                        if(elementTypesOfExtendedTypeList.item(i)!=null&&elementTypesOfExtendedTypeList.item(i).getNodeName()!=null&&
                            ...
                            //-System.out.println("### parseComplexContentForDocumentType ###");
                            parseSequenceOrChoiceNode(theWSDLxsdSchema,elementTypesOfExtendedTypeList.item(i), iterNumber, co);                            
    ...
    private static void parseSequenceOrChoiceNode(javax.wsdl.extensions.schema.Schema theWSDLxsdSchema,Node inputNode, int iterNumber,ComplexObject co) {
        
        //-System.out.println("\tparse sequence for Node: "+inputNode.getNodeName()+"    "+co.objectName+"   ##  "+                co.getHasComplexObjects().size()+" "+co.getHasNativeObjects().size());
        ...
        try{
            //-System.out.println("Node name:"+ inputNode.getAttributes().getNamedItem("name").getNodeValue());
        }catch(Exception e){
        ...
        if(listOfChildrenOfTheSequenceNode!=null){
            ...
            for(int k2=0;k2<listOfChildrenOfTheSequenceNode.getLength();k2++){
                ...
                if(!listOfChildrenOfTheSequenceNode.item(k2).hasAttributes())continue;
                ...
                if(listOfChildrenOfTheSequenceNode.item(k2).getNodeName().contains("element")||
                    ...
                    if(!referencedType.equals("")){
                        ...
                        if(xsdNamespace!=null){                                                                      
                            ...
                            //parseImportedXSDforLiteral3(s1, type1, iterNumber+1,true,co,
                            //        xsdNamespace);
                            ...
                            ////-System.out.println("Namespace was null... Will be treated as NATIVE TYPE...");
                             ...
                             //MitsosParser.parsedObjectsHashmap.put(newNativeObject.objectType, newNativeObject);
                       ...
                       // //-ta.append("\n\t\t\tType: "+attType+"  ("+additionalInfo+")");
                        ...
                        if(attType.startsWith(MitsosParser.targetNamespacePrefix)){ 
                            String type1=attType.substring(MitsosParser.targetNamespacePrefix.length(), attType.length());
                           ...
                           //     //-ta.append("  ("+type1+"[])");          
                            ...
                            String xsdNamespace = (String)MitsosParser.namespaces.get(MitsosParser.targetNamespacePrefix.replace(":", ""));
                            //parseTypeIterativeForXSDImport(s1, type1, iterNumber+1,true,newComplexObject);     
                            if(xsdNamespace!=null){
                                if(!objectAlreadyExistsInHashMap){
                                    MitsosParser.parsedObjectsHashmap.put(newComplexObject.getObjectType(), newComplexObject);
                                    parseTypeIterativeFromSpecificNamespace(theWSDLxsdSchema, null,xsdNamespace, type1, newComplexObject); 
                                    co.getHasComplexObjects().add(newComplexObject);
                                }else{
                                    try{
                            ...
                            if(MitsosParser.nativeTypePrefix!=null&&attType.startsWith(MitsosParser.nativeTypePrefix)){
                                 NativeObject newNativeObject=new NativeObject();
                                 newNativeObject.setObjectName(new QName(attName));
                                 newNativeObject.setAdditionalInfo(additionalInfo);
                                 newNativeObject.setObjectType(new QName(attType));
                                ...
                                if(attType.contains(":")){
                                    ...
                                    if(xsdNamespace!=null){
                                        ...
                                        ////-System.out.println("Namespace Found!!!");   
                                            ...
                                            boolean objectExistsInHashmap=MitsosParser.parsedObjectsHashmap.containsKey(type1);
                    ...
                    //-System.out.println("AAA");
                    //EDW PREPEI NA ELEGKSW AN sto co yparxoun ComplexObjects me mono ena NativeObject mesa
                    //Ayta einai stin ousia NativeObjects

                }else {//EINAI SIMPLE TYPE
                    //XAZOMARA EINAI AYTO MALLON, PREPEI na einai element, opote se normal synthikes den tha mpei edw        
                ...
                //-System.out.println("\tFinished: "+inputNode.getNodeName()+"    "+co.objectName+"   ##  "+                co.getHasComplexObjects().size()+" "+co.getHasNativeObjects().size());
    ...
    private static void parseTypeFromElementNodeOfXSD(javax.wsdl.extensions.schema.Schema theWSDLxsdSchema, Node n, ComplexObject co){
        ...
        if(!foundChildren){
            ...
            if(n.getAttributes()!=null&&(n.getAttributes().getNamedItem("type")!=null||n.getAttributes().getNamedItem("ref")!=null)){                
                String typeA=null;
                             ...
                             for(int i=0;i<nnm.getLength();i++){
                                 if(nnm.item(i)!=null){
                                     if(nnm.item(i).getNodeName()!=null){
                                         if(!nnm.item(i).getNodeName().equals("name")&&!nnm.item(i).getNodeName().equals("type")){
    ...
    private static void parseTypeFromComplexTypeNodeOfXSD(javax.wsdl.extensions.schema.Schema theWSDLxsdSchema, Node n, 
        ...
        /*
        try{
            if(n.getAttributes().getNamedItem("type").equals("AdministrativeAreaCodeType")){
                //-System.out.println("AAA");
            }
        }catch(Exception e){
        ...
        if(listWithChildrenOfTheComplexTypeNode!=null){
            for(int k=0;k<listWithChildrenOfTheComplexTypeNode.getLength();k++){
                ...
                if(listWithChildrenOfTheComplexTypeNode.item(k).getNodeName().contains("sequence")||
                    ...
                    parseComplexContentForDocumentType(theWSDLxsdSchema,listWithChildrenOfTheComplexTypeNode.item(k), 0, co);
                }else if(listWithChildrenOfTheComplexTypeNode.item(k).getNodeName().contains("simpleContent")){
                    parseSimpleContentForDocumentType(theWSDLxsdSchema,listWithChildrenOfTheComplexTypeNode.item(k), 0, co);
                }
            }
        } 
    ...
    //            ////-System.out.println("\t"+childrenOfTheDamned.item(k).getNodeName());
     ...
     private static void parseTypeIterativeFromSpecificNamespace(javax.wsdl.extensions.schema.Schema theWSDLxsdSchema, Schema currentSchema,
            String xsdNamespace, String type1, ComplexObject co){
        //TO PERIEXOMENO TOU type1 pou tha vrw prepei na mpei mesa sto co
        //an to type1 periexei complexObjects, nativeObjects etc. tha ftiaksw edw kainourgia kai tha ta valw mesa sto co
        ...
        if(currentSchema==null){
            theImportedSchema = WSDL_XSD_SchemaToolkit.getTheImportedOrIncludedSchemaWithTheSpecificNS(theWSDLxsdSchema, xsdNamespace);
        }else{
            theImportedSchema = currentSchema;
        }
        if(theImportedSchema==null)theImportedSchema=theWSDLxsdSchema; 
        
        //-System.out.println("WWW::  "+theImportedSchema.getDocumentBaseURI());
        org.w3c.dom.Element impSchElem = theImportedSchema.getElement();                
        if(impSchElem==null)return;
        ...
        if(elementNodesList!=null&&complexTypeNodesList!=null&&simpleTypeNodesList!=null&&groupTypeNodesList!=null){
        ...
        if(includesList!=null){
            ...
            while(includesIter.hasNext()){
                ...
                if(WSDL_XSD_SchemaToolkit.nodeExistsInSchema(type1, includedSchema)){
                    //-System.out.println("### Looking in schema: "+includedSchema.getDocumentBaseURI());
                    parseTypeIterativeFromSpecificNamespace(theWSDLxsdSchema, includedSchema, xsdNamespace, type1, co);
                    return;
                }
            }
        }

        Map importsMap=theImportedSchema.getImports();
        if(importsMap!=null){
            ...
            while(importsIter.hasNext()){
                ...
                if(WSDL_XSD_SchemaToolkit.nodeExistsInSchema(type1, importedSchema)){
                    parseTypeIterativeFromSpecificNamespace(theWSDLxsdSchema, importedSchema, xsdNamespace, type1, co);
                    return;
                }                          
            }
        }*/
    ...
    public static void parseDocumentType2(Definition definition, QName inPartType,  WSOperationInput mitsosOperationInput, 
        ...
        if(extElementsList!=null){
            ...
            while(iter1.hasNext()){
                try{
                    com.ibm.wsdl.extensions.schema.SchemaImpl theWSDLxsdSchema = (com.ibm.wsdl.extensions.schema.SchemaImpl)iter1.next();
                    //-System.out.println(theWSDLxsdSchema.toString());     
                    //-System.out.println(theWSDLxsdSchema.getElementType().getLocalPart());
                    /*                                                            
                    parseTypeIterativeFromSpecificNamespace(javax.wsdl.extensions.schema.Schema theWSDLxsdSchema, Schema currentSchema,
            String xsdNamespace, String type1, ComplexObject co){
                    */
                    ...
                    if(elementNodesList!=null&&complexTypeNodesList!=null&&simpleTypeNodesList!=null&&groupTypeNodesList!=null){
                        if(elementNodesList.getLength()==0&&complexTypeNodesList.getLength()==0&&
                            ...
                            complexTypeNodesList = e1.getElementsByTagName(MitsosParser.nativeTypePrefix+"complexType");                    
                            ...
                            if(elementNodesList.getLength()==0&&complexTypeNodesList.getLength()==0&&
                                simpleTypeNodesList.getLength()==0&&groupTypeNodesList.getLength()==0){
                                //Cover tin periptwsi pou yparxoun tags p.x. xs:element, enw to Namespace
                                //tou XML (nativeNamespacePrefix) einai diaforetiko (p.x. s:)
                                Vector xmlSchemaKeys=new Vector();                        
                                Set entries=MitsosParser.namespaces.entrySet();                        
                    ...
                    if(elementNodesList!=null && !typeParsingFinished){
                        for(int i=0;i<elementNodesList.getLength();i++){
                            Node n=elementNodesList.item(i);
                            if(n.hasAttributes()&&n.getAttributes().getNamedItem("name")!=null){
                                if(n.getAttributes().getNamedItem("name").getNodeValue().equals(inPartType.getLocalPart())){                                    
                    ...
                    if(prefix!=null){
                        n=WSDL_XSD_SchemaToolkit.getNodeOfType(
                                    prefix+":"+inPartType.getLocalPart(), theWSDLxsdSchema, null);
                    }else{                    
                        if(inPartType.getPrefix()!=null&&inPartType.getPrefix().length()>0){
                            n=WSDL_XSD_SchemaToolkit.getNodeOfType(
                            ...
                            n=WSDL_XSD_SchemaToolkit.getNodeOfType(
                                    inPartType.getLocalPart(), theWSDLxsdSchema, null);
                        }
                    }
            ...
            if(part.getName()!=null&&part.getName().equals("parameters")){
                ...
                if(mitsosOperationInput!=null){     
                    ...
                    Iterator nativeObjsIter=dummyCo.getHasNativeObjects().iterator();
                    while(nativeObjsIter.hasNext()){                    
                        mitsosOperationInput.hasNativeOrComplexObjects.add(nativeObjsIter.next());
                    }
                    Iterator complexObjsIter=dummyCo.getHasComplexObjects().iterator();
                    while(complexObjsIter.hasNext()){                    
                        mitsosOperationInput.hasNativeOrComplexObjects.add(complexObjsIter.next());
                    }*/
                }else if(mitsosOperationOutput!=null){          
                    if(dummyCo.getHasComplexObjects().size()==0&&dummyCo.getHasNativeObjects().size()==1){
                        NativeObject no1=(NativeObject)dummyCo.getHasNativeObjects().get(0);
                    ...
                    Iterator nativeObjsIter=dummyCo.getHasNativeObjects().iterator();
                    while(nativeObjsIter.hasNext()){
                        mitsosOperationOutput.hasNativeOrComplexObjects.add(nativeObjsIter.next());
                    }
                    Iterator complexObjsIter=dummyCo.getHasComplexObjects().iterator();
                    while(complexObjsIter.hasNext()){                    
                        mitsosOperationOutput.hasNativeOrComplexObjects.add(complexObjsIter.next());
                    }*/
                }
            }
    ...
    public static void parseDocumentType_Deprecated(Definition definition, QName inPartType,  WSOperationInput mitsosOperationInput, 
                    ...
                    for(int i=0;i<children.getLength();i++){
                        ...
                        if(n.getNodeName()!=null&&n.getNodeName().contains("import")){
                            ...
                            if(n.getAttributes()!=null&&
                                ...
                                parseImportedXSDforLiteral(s1, inPartType, mitsosOperationInput,mitsosOperationOutput,inPartType.getNamespaceURI());
                        ...
                        if(n.getAttributes()!=null&&n.getAttributes().getNamedItem("name")!=null&&
                                n.getAttributes().getNamedItem("name").getNodeValue()!=null&&
                                n.getAttributes().getNamedItem("name").getNodeValue().equals(inPartType.getLocalPart())){
        ...
        //MitsosParser.parseExtElements(definition, inPartType, ta, null, null);
    }
    
    private static void parseImportedXSDforLiteral3(javax.wsdl.extensions.schema.Schema theXSDschemaDefinedInTheWSDL, String typeName, int iterNumber, 
                        ...
                        for(int i=0;i<elementNodesList.getLength();i++){
                            ...
                            if(n.hasAttributes()&&n.getAttributes().getNamedItem("name")!=null){
                                if(n.getAttributes().getNamedItem("name").getNodeValue().equals(typeName)){   
                                    ...
                                    typeParsingFinished=true;
                                    break;//Stamataei so psaksimo sta elementNodes
                                }
                            }
                        }
                        if(typeParsingFinished)break;//Stamataei to psaksimo
                    }
     ...
     private static void parseImportedXSDforLiteral(SchemaImpl schemaImpl, QName inPartType, 
        ...
        while (importsItt.hasNext()) {
         ...
         //       ////-System.out.println(schemaImport.getNamespaceURI());
                if(schemaImport.getNamespaceURI()!=null&&schemaImport.getNamespaceURI().equals(xsdImportNamespace)){
                    javax.wsdl.extensions.schema.Schema importedSchema = schemaImport.getReferencedSchema();                
                    org.w3c.dom.Element impSchElem=importedSchema.getElement();
                    if(impSchElem==null)continue;
...
//PSAXNW GIA TO TYPE MESA STO definition
                                                  //      ////-System.out.println("#########################################  COMPLEX!!!! ITERATIVE");
                                                        ...
                                                        Node parsedAttribute=parseTypeIterativeForXSDImport(importedSchema,type1,0,true,co);
                                               ...
                                               //                         ////-System.out.println("Namespace Found!!!");
                                                                        ...
                                                                        //AN to type1 anhkei se allo Namespace...
                                                                        parseImportedXSDforLiteral3(schemaImpl, type1, 1,true,newComplexObject,xsdNamespace);
                                                                        
                                                                        if(operationInputs!=null){                                                       
                                                                            operationInputs.getHasNativeOrComplexObjects().add(newComplexObject);
                                                                        }else if(operationOutputs!=null){                                                    
                                                                            operationOutputs.getHasNativeOrComplexObjects().add(newComplexObject);
                                                                        } 
     ...
     private static void parseIncludedXSDforLiteral(SchemaImpl schemaImpl, QName inPartType, 
        ...
        //Iterator importsItt = importsMap.values().iterator();
        while (importsItt.hasNext()) {
            com.ibm.wsdl.extensions.schema.SchemaReferenceImpl schemaImport= (com.ibm.wsdl.extensions.schema.SchemaReferenceImpl) importsItt.next();
                ...
                if(childrenOfChildOfSchema!=null){
                    for(int j=0;j<childrenOfChildOfSchema.getLength();j++){
                        ...
                        if(n1.getAttributes().getNamedItem("name")!=null&&
                                n1.getAttributes().getNamedItem("name").getNodeValue()!=null&&
                                n1.getAttributes().getNamedItem("name").getNodeValue().equals(inPartType.getLocalPart())){ //VRETHIKE TO TYPE!!!!!!!
                            ...
                            if(childrenOfTheDamned!=null){
                                if(childrenOfTheDamned.getLength()>0){
                                    ...
                                    if(n1.getAttributes().getNamedItem("type").getNodeValue()!=null){
                                        ...
                                        if(n1.getAttributes().getNamedItem("type").getNodeValue().startsWith(MitsosParser.targetNamespacePrefix)){//COMPLEX TYPE
                                            //PSAXNW GIA TO TYPE MESA STO definition
                                            //////-System.out.println("#########################################  COMPLEX!!!! ITERATIVE");
                                            String type1=attType.substring(4, attType.length());
                                            ...
                                            if(MitsosParser.nativeTypePrefix!=null&&n1.getAttributes().getNamedItem("type").getNodeValue().startsWith(MitsosParser.nativeTypePrefix)){
                                            
                                            }else{//to namespace parapempei se allo imported i included XSD
                                                ...
                                                //////-System.out.println("Looking for Namespace with prefix: "+n1.getAttributes().getNamedItem("type").getNodeValue());
                                                    for(int i=0;i<col.size();i++){
                                                        ...
                                                        while(iter1.hasNext()&&keyIterator.hasNext()){
                                                            ...
                                                            if(key.contains(n1.getAttributes().getNamedItem("type").getNodeValue())){
                                                                //////-System.out.println("GOT IT!!!... namespace: "+s);
                                                                //CHECK Imports / Includes for the namespace found HERE...
     ...
     public static Node parseTypeIterativeForXSDImport(javax.wsdl.extensions.schema.Schema s1, String typeName,  int iterNumber, 
                                    ...
                                    for(int k=0;k<childrenOfTheDamned.getLength();k++){
                                        ...
                                        if(childrenOfTheDamned.item(k).getNodeName().contains("element")){
                                            ...
                                            if(attType.startsWith(MitsosParser.targetNamespacePrefix)){ 
                                                ...
                                                parseTypeIterativeForXSDImport(s1, type1,  iterNumber+1,fromLiteral,newComplexObject);        
