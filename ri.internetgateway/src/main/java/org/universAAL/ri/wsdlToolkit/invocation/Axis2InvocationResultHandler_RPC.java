...
public class Axis2InvocationResultHandler_RPC {
		...
		while (parsedOperationOutputsIter.hasNext()) {
			...
			if (parsedOutObj instanceof NativeObject) {
				...
				if (!outCO.isIsArrayType()) {
					...
					Iterator<OMElementImpl> omElement = findTheArrayObjectNodesInTheResponseBody(
							inMsgCtx, null, outCO);
					int i = 0;
		...
		try {
			if (co.getHasNativeObjects() != null
				...
				NativeObject no1 = (NativeObject) co.getHasNativeObjects().get(
						0);
				Iterator<OMElementImpl> vec = findTheArrayObjectNodesInOMElement(
						omElement, no1, null);
				int i = 0;
				while (vec.hasNext()) {
					if (i == 0) {
			...
			try {
				...
				while (it2.hasNext()) {
					...
					try {
						...
						// OMElementImpl childOMElement2 = (OMElementImpl)
						// it2.next();
						if (childOMElement2.getQName().getLocalPart()
								.equals(objectQName.getLocalPart())) {
							return childOMElement2;
	...
	private static Iterator<OMElementImpl> findTheArrayObjectNodesInOMElement(
