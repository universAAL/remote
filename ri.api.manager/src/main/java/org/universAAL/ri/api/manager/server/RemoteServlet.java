...
public class RemoteServlet extends javax.servlet.http.HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		if (Configuration.getGETenabled()) {
			...
			resp.sendError(HttpServletResponse.SC_METHOD_NOT_ALLOWED, "HTTP GET Not allowed");
		}
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (Activator.isHardcoded()) {
			...
			// intercepted by HttpContext (Authenticator) and if it fails it
			// will
			// not get this far.
		...
		if (user == null || method == null || param == null) {
			...
			Activator.logE("doPost", "STATS failed " + method + " for " + req.toString() + " in "
					+ (System.currentTimeMillis() - tst) + " because Missing parameter");
			return;
		}

		if (remoteAPI == null) {
			resp.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
			resp.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "API not available");
			Activator.logE("doPost", "STATS failed " + method + " for " + req.toString() + " in "
					+ (System.currentTimeMillis() - tst) + " because API not available");
			return;
		} else {
			String body = null;
			try {
				if (RemoteAPI.METHOD_REGISTER.equals(method)) {
					...
					resp.setStatus(HttpServletResponse.SC_BAD_REQUEST);
					resp.sendError(HttpServletResponse.SC_BAD_REQUEST, "No such method");
					Activator.logE("doPost", "STATS failed " + method + " for " + req.toString() + " in "
							+ (System.currentTimeMillis() - tst) + " because No such method");
					return;
				}
			} catch (Exception e) {
				resp.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
				...
				Activator.logE("doPost", "STATS failed " + method + " for " + req.toString() + " in "
						+ (System.currentTimeMillis() - tst) + " because " + e);
				return;
			}
			...
			os.flush();
			os.close();
			Activator.logD("doPost", "STATS Serviced " + method + " for " + req.toString() + " in "
