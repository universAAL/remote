...
public class Checker extends TimerTask {
	...
	public void run() {
		...
		HashMap<String, Long> current = (HashMap<String, Long>) CSubscriber.getLastknownof().clone();
		if (!current.isEmpty()) {
			Iterator<String> keys = current.keySet().iterator();
			while (keys.hasNext()) {
				String key = keys.next();
				Long tst = current.get(key);
				if ((now - tst) > (Activator.multiplier * 3600000)) {// If
																		// lastknown
																		// tst
																		// is
																		// before
					...
					if (!CSubscriber.getMissing().contains(key)) {
						LogUtils.logInfo(Activator.context, Checker.class, "run",
								"Missing keep-alive signal from tenant " + key);
						CSubscriber.getMissing().add(key);
						if (!SendMail.send(key, now)) {
							LogUtils.logInfo(Activator.context, Checker.class, "run",
									"The warning email about tenant " + key + " was not sent");
						}
					}
				}
			}
