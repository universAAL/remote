...
public class MitsosParser {
				...
				while (iter1.hasNext()) {
					try {
						...
						while (importsItt.hasNext()) {
							...
							/*
							 * parseImportedXSDforLiteral(s1, inPartType, ta,
							 * mitsosOperationInput
							 * ,mitsosOperationOutput,inPartType
							 * .getNamespaceURI());
							 * //parseImportedXSDusingCastor
							 ...
							 * ("schemaLocation")!=null&&
							 * n.getAttributes().getNamedItem
							 * ("schemaLocation").getNodeValue()!=null){
							 * ////-System
							 * .out.println("FOUND AN XSD INCLUDE!!!");
							 * ////-System
							 * .out.println(n.getAttributes().getNamedItem
		...
		for (int i = 0; i < l1.size(); i++) {
			...
			try {
				...
				try {
					...
					// httpbimpl=(com.ibm.wsdl.extensions.http.HTTPBindingImpl)l1.get(i);
					...
					for (int i11 = 0; i11 < l11.size(); i11++) {
						...
						try {
							...
							try {
								...
								com.ibm.wsdl.extensions.http.HTTPBindingImpl httpbimpl = (com.ibm.wsdl.extensions.http.HTTPBindingImpl) l1
										.get(i);
								// ////-System.out.println(httpbimpl.toString());
								bindingStyle = "http";
							}
					...
					for (int f1 = 0; f1 < extensibilityElementsList.size(); f1++) {
						...
						try {
							...
							try {
								...
								// e1.printStackTrace();
								try {
									com.ibm.wsdl.extensions.mime.MIMEContentImpl mimecontimpl = (com.ibm.wsdl.extensions.mime.MIMEContentImpl) extensibilityElementsList
											.get(f1);
	...
	private static void parseImportedXSD(
		...
		while (importsItt.hasNext()) {
			...
			while (importsItt2.hasNext()) {
				...
				/*
				 * Attr
				 * attr=impSchElem.getAttributeNode(inPartType.getLocalPart());
				 * NodeList
				 * nl=impSchElem.getElementsByTagName("xs:complexType"); for(int
				 * i=0;i<nl.getLength();i++){ Node nn1=nl.item(i);
	 ...
	 * schemaImport=schemaImportInitial; }else{
	 * javax.wsdl.extensions.schema.Schema
	 * s11=schemaImportInitial.getReferencedSchema(); Map importsMap1 =
	 * s11.getImports(); Iterator importsItt1 = importsMap1.values().iterator();
	 ...
	 * //NodeList
	 * simpleTypeNodeList=impSchElem.getElementsByTagName("simpleType");
	 ...
	 * attributesOfType
	 * =childrenOfTheChildrenOfTheDamned.item(k2).getAttributes(); //
	 * ////-System
	 ...
	 * ////-System.out.println("\tName: "+att.getNodeValue());
	 * attName=att.getNodeValue(); }else
	 * if(att.getNodeName().equalsIgnoreCase("type")){ //
	 * ////-System.out.println("\tType: "+att.getNodeValue());
	 ...
	 * type1.length()-5); //-ta.append("  ("+type1+"[])");
	 * newComplexObject.objectType=type1+"[]"; } else
	 ...
	 * }else{ if(attType.contains(":")){ String key1=attType.substring(0,
	 * attType.indexOf(":")); String xsdNamespace=(String)namespaces.get(key1);
	 ...
	 * for(int k=0;k<childrenOfTheDamned.getLength();k++){ //
	 * //-System.out.println("\t"+childrenOfTheDamned.item(k).getNodeName());
	 * if(childrenOfTheDamned.item(k).getNodeName().contains("sequence")||
	 ...
	 * importsList.iterator(); while (importsItt2.hasNext()) { SchemaImportImpl
	 * schemaImport = (SchemaImportImpl) importsItt2.next();
	 * javax.wsdl.extensions.schema.Schema importedSchema =
	 * schemaImport.getReferencedSchema(); org.w3c.dom.Element
	 * impSchElem=importedSchema.getElement(); if(impSchElem==null)continue;
	 * 
	 ...
	 * operationInputs.getHasNativeOrComplexObjects().add(no); }else
	 * if(operationOutputs!=null){ NativeObject no=new NativeObject();
	 * no.objectName=attName; no.objectType=attType;
	 * no.additionalInfo=additionalInfo;
	 * operationOutputs.getHasNativeOrComplexObjects().add(no); } }else
	 * if(attType.startsWith(targetNamespacePrefix)){ //PSAXNW GIA TO TYPE MESA
	...
	private static void parseOperation(WSOperation mitsosOperation,
		...
		} else if (bindingStyle.equals("http"))
			operationUse = "http";
		...
		while (inputPartIter.hasNext()) {
			...
			if (operationUse.equals("encoded")) {
				if (bindingStyle.equals("rpc")) {
					try {
						 ...
						 * if(stringToAppendForArray==null){//DEN EINAI ARRAY
						 * if(
						 * inPartType.getNamespaceURI().equals(namespaces.get(
						 * nativeTypePrefix.replace(":", "")))){ //IT IS A
						 * NATIVE TYPE
						 * mitsosOperationInput.getHasNativeOrComplexObjects
						 * ().add(no); }else{ //IT IS A COMPLEX TYPE
						 * mitsosOperationInput
						 * .getHasNativeOrComplexObjects().add(co); } }else{
						 * if(inPartType
						 * .getNamespaceURI().equals(namespaces.get(
						 * nativeTypePrefix.replace(":", "")))){ //IT IS A
						 * NATIVE TYPE
						 * mitsosOperationInput.getHasNativeOrComplexObjects
						 * ().add(no); }else{ //IT IS A COMPLEX TYPE
			...
			/*
			 * try{ String inPartName = part.getName();
			 * //-ta.append("\n\n\t\t\tName: "+inPartName); QName
			 ...
			 * substring(0,inPartType.getLocalPart().length()-5) +"[] )"); }else
			 * if(inPartType.getLocalPart()!=null&&inPartType.getLocalPart().
			 * startsWith("ArrayOf")){
			 * //-ta.append("  ( "+inPartType.getLocalPart
			 * ().replaceFirst("ArrayOf", "")+"[] )"); }else
		...
		while (outputPartIter.hasNext()) {
			...
			if (operationUse.equals("encoded")) {
				...
				try {
					...
					e.printStackTrace();
					/*
					 * //-ta.append("\n\t\t\tType (El): "+part.getElementName().
					 * getLocalPart());
					 * if(part.getElementName().getLocalPart()!=
					 * null&&part.getElementName
					 * ().getLocalPart().endsWith("Array")){
					 * //-ta.append("  ( "+
					 * part.getElementName().getLocalPart().substring
					 * (0,part.getElementName().getLocalPart().length()-5)
					 * +"[] )"); }else
					 * if(part.getElementName().getLocalPart()!=null
					 * &&part.getElementName
					 * ().getLocalPart().startsWith("ArrayOf")){
					 * //-ta.append("  ( "
					 * +part.getElementName().getLocalPart().replaceFirst
					 * ("ArrayOf", "")+"[] )"); }else
					 * if(part.getElementName().getLocalPart
					 * ()!=null&&part.getElementName
					 * ().getLocalPart().endsWith("[]")){
					 * //-ta.append("  ( "+part
		...
		// if
		// (mi.getHasNativeOrComplexObjects().get(i).getClass().toString().contains("NativeObject")){
		...
		// if
		// (mo.getHasNativeOrComplexObjects().get(i).getClass().toString().contains("NativeObject")){
		// NativeObject no = (NativeObject)
		...
		if (nativeAttributeNamesList != null) {
			...
			while (iter1.hasNext()) {
				try {
					...
					// EDW PREPEI NA MPEI KWDIKAS GIA NA ELEGXEI AN EINAI TO
					// EPITHYMITO (An exei to idio onoma me tin eisodo pou
					// irthe)
				} catch (Exception e) {
					e.printStackTrace();
				}
			...
			while (iter1.hasNext()) {
				try {
					...
					for (int i = 0; i < children.getLength(); i++) {
						...
						/*
						 * try{ ////-System.out.println("FFFFFFFFFFFFFF");
						 * ////-System
	...
	private static void parseExtendedType(
		...
		try {
			// com.ibm.wsdl.extensions.schema.SchemaImpl s1 =
			// (com.ibm.wsdl.extensions.schema.SchemaImpl)iter1.next();
			// //-System.out.println(s1.toString());
