...
public class PersistenceMySQL implements Persistence {
	...
	public void init(RemoteAPI remoteAPI, ModuleContext context) {
		this.api = remoteAPI;
		dbURL = "jdbc:mysql:" + Configuration.getDBPath(context);
		dbUSR = Configuration.getDBUser();
		dbPWD = Configuration.getDBPass();
		...
		try {
			...
			String createDB = "CREATE DATABASE IF NOT EXISTS " + DBNAME + " DEFAULT CHARACTER SET utf8 ";
			String createDBPWD = "CREATE DATABASE IF NOT EXISTS " + PWDDBNAME + " DEFAULT CHARACTER SET utf8 ";
			...
			String createSUBSCRIBERS = "CREATE TABLE IF NOT EXISTS " + DBNAME
					...
					+ " tstmp TIMESTAMP, CONSTRAINT subid_fk FOREIGN KEY (id) REFERENCES " + DBNAME
					+ ".registers(id) ON DELETE CASCADE)";
			String createCALLEES = "CREATE TABLE IF NOT EXISTS " + DBNAME
					...
					+ " tstmp TIMESTAMP, CONSTRAINT calid_fk FOREIGN KEY (id) REFERENCES " + DBNAME
					+ ".registers(id) ON DELETE CASCADE)";
			String createPWDS = "CREATE TABLE IF NOT EXISTS " + PWDDBNAME
					+ ".pwds ( id VARCHAR(254) PRIMARY KEY NOT NULL, pwd VARCHAR(100) )";
			// create db and tables. I dont trust addBatch. TODO AutoCommit
			// false?
	...
	public void storeRegister(String id, String remote, String v) {
		...
		if (checkUserFromREGISTERS(id)) {
			...
			storeREGISTERS = "UPDATE " + DBNAME + ".registers SET remote='" + remote + "', tstmp='" + t + "'"
					+ (v != null ? ", version='" + v + "'" : "") + " WHERE id='" + id + "'";
		} else {
			// New id - > insert all
			storeREGISTERS = "INSERT INTO " + DBNAME + ".registers (id, remote, tstmp, version) VALUES ('" + id + "','"
	...
	public void storeSubscriber(String id, String pattern) {
		Timestamp t = new Timestamp(System.currentTimeMillis());
		String storeREGISTERS = "INSERT INTO " + DBNAME + ".subscribers (id, pattern, tstmp) VALUES ('" + id + "','"
				+ pattern + "','" + t.toString() + "')";
		executeGeneric(storeREGISTERS);
	}
	...
	public void storeCallee(String id, String profile) {
		Timestamp t = new Timestamp(System.currentTimeMillis());
		String storeREGISTERS = "INSERT INTO " + DBNAME + ".callees (id, profile, tstmp) VALUES ('" + id + "','"
				+ profile + "','" + t.toString() + "')";
		executeGeneric(storeREGISTERS);
	}
	...
	public void removeRegister(String id) {
		...
		try {
			...
			stmt.executeUpdate(deleteREGISTERS);// TODO Remove in cascade? Rsult
	...
	public void removeOlderThan(Long millis) {
		...
		try {
			...
			String deleteCALLEES = "DELETE FROM " + DBNAME + ".callees WHERE {fn TIMESTAMPDIFF( SQL_TSI_DAY, tstmp, '"
					+ t.toString() + "')} > 0";
			String deleteSUBSCRIBERS = "delete from " + DBNAME
					+ ".subscribers WHERE {fn TIMESTAMPDIFF( SQL_TSI_DAY, tstmp, '" + t.toString() + "')} > 0";
			String deleteREGISTERS = "delete from " + DBNAME
					+ ".registers WHERE {fn TIMESTAMPDIFF( SQL_TSI_DAY, tstmp, '" + t.toString() + "')} > 0";
			// create tables. Again, put individual try/catch to keep deleting
			// if errors. I dont trust addBatch.
			stmt.executeUpdate(deleteCALLEES);
			stmt.executeUpdate(deleteSUBSCRIBERS);
	...
	public void storeUserPWD(String id, String pwd) {
		String storeREGISTERS = "INSERT INTO " + PWDDBNAME + ".pwds (id, pwd) VALUES ('" + id + "','" + pwd + "')";
		executeGeneric(storeREGISTERS);
	}
