...
public class Axis2InvocationResultHandler_RpcEncoded {
    ...
    private static OMElementImpl findOMNode_RpcEncoded_AmongChildren_ITERATIVE(SOAPBody body, OMElementImpl inputNode, QName resultQName){
        if(inputNode==null || resultQName==null)return null;

        //Iterator iter1=inputNode.getChildrenWithLocalName(resultQName.getLocalPart());
    ...
    private static OMElementImpl findOMNodeWithTheSpecific_ID_AmongChildren_ITERATIVE(OMElementImpl omElement, SOAPBody body, String idToFind){
        
        if((body==null && omElement==null)|| idToFind==null)return null;
        ...
        if(body!=null){
            ...
            while(iter2.hasNext()){
                ...
                if(obj.getClass().getName().contains("org.apache.axiom.om.impl.llom.OMTextImpl")){
                    ...
                    org.apache.axiom.om.impl.llom.OMElementImpl childOMElement=(OMElementImpl)obj;
                    OMElementImpl result=findOMNodeWithTheSpecific_ID_AmongChildren_ITERATIVE(childOMElement, null, idToFind);
                    return result;
                }
            }
    ...
    private static OMElementImpl findTheObjectNodeInTheRpc_EncodedResponseBody(MessageContext inMsgCtx, NativeObject no, ComplexObject co){

        if(inMsgCtx==null||inMsgCtx.getEnvelope()==null||inMsgCtx.getEnvelope().getBody()==null)return null;
        ...
        //An den exei vrethei mesa sta children tou body, search mesa sta children twn children tou body (ITERATIVE!!!)
        Iterator iter2=body.getChildren();
        while(iter2.hasNext()){
    ...
    private static void setNativeObjectValue(NativeObject no, OMElementImpl omElement){
        ...
        if(omElement==null||no==null)return;
    ...
    private static void setComplexObjectValues(SOAPBody soapBody, ComplexObject co, OMElementImpl omElement){
        ...
        if(omElement==null||co==null)return;
        ...
        if(!co.isIsArrayType()){
            if(co.getHasComplexObjects()!=null&&co.getHasComplexObjects().size()>0){
                ...
                while(cosIter.hasNext()){
                    ...
                    if(!co1.isIsArrayType()){
                        OMElementImpl omElement1=findOMNode_RpcEncoded_AmongChildren_ITERATIVE(soapBody, omElement, co1.getObjectName());
                        //OMElementImpl omElement1=findTheObjectNodeInTheResponseBody(inMsgCtx, null, co1);
            ...
            if(co.getHasNativeObjects()!=null&&co.getHasNativeObjects().size()>0){
                ...
                while(nosIter.hasNext()){
                    ...
                    OMElementImpl omElement1=findOMNode_RpcEncoded_AmongChildren_ITERATIVE(soapBody, omElement, no1.getObjectName());
    ...
    private static void setArrayObjectValues(SOAPBody soapBody, ComplexObject co, OMElementImpl inputOmElement){
        ...
        if(inputOmElement==null)return;
        if((inputOmElement.getLocalName()!=null&&inputOmElement.getLocalName().equals(co.getObjectName().getLocalPart()))){
        ...
        while(childrenElementsIter.hasNext()){
            try{
                ...
                if(att!=null&&att.getAttributeValue()!=null){
                    String hasReferenceId=att.getAttributeValue();
                    OMElementImpl element111=findOMNodeWithTheSpecific_ID_AmongChildren_ITERATIVE(null, soapBody, hasReferenceId);
                    if(element111!=null){
        ...
        if(co.getHasComplexObjects()!=null&&co.getHasComplexObjects().size()>0){
            ...
            Iterator cosIter=co.getHasComplexObjects().iterator();
            while(cosIter.hasNext()){
                //to iteration edw tha exei mono mia epanalipsi (tha treksei mono mia fora)
                co1=(ComplexObject)cosIter.next();
            }

            //Parse ComplexObjects of response
            if(co1!=null){                                
                //Iterator iter1=findOMNodesAmongChildren_ITERATIVE(element, co1.objectName);

                Iterator iter1=arrayCantainedElems.iterator();
                if(iter1!=null){
                    //Edw tha mpei mono an yparxei domi p.x. "<hasResources><helpResource/><helpResource/></hasResources>"
                    boolean firstRun=true;
                    while(iter1.hasNext()){
                        org.apache.axiom.om.impl.llom.OMElementImpl elem=
                            (org.apache.axiom.om.impl.llom.OMElementImpl)iter1.next();
                        if(firstRun){
                    ...
                    //Edw mporei na mpei an yparxei domi p.x. "<hasResources><helpResource/></hasResources>" xwris na yparxoun ta node <helpResource>,</helpResource>
