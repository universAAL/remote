...
public class ArtifactIT extends IntegrationTest {
    ...
    private static final String CEVENT="@prefix ph: <http://ontology.universaal.org/PhThing.owl#> ." +
    		...
    		" a ctxt:ContextEvent ;" +
    		" rdf:subject tst:controlledLamp1 ;" +
    		...
    		" light:srcBrightness \"100\"^^xsd:int ." +
    ...
    private static final String CPATTERN="@prefix rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#> ." +
    		...
    		" :onProperty rdf:subject ] , [ :hasValue ns:srcBrightness ;" +
    		" a :Restriction ;" +
    		" :onProperty rdf:predicate ] , [ a :Restriction ;" +
    ...
    private static final String SPROFILE="@prefix ns: <http://www.daml.org/services/owl-s/1.1/Profile.owl#> ." +
    		...
    		" _:BN000000 ns2:presentedBy ns1:turnOn ;" +
    		" a ns:Profile ;" +
    		" ns:has_process ns1:turnOnProcess ;" +
    		" ns:hasResult [ a :Result ;" +
    		" :hasEffect [ psn:affectedProperty [ a psn:PropertyPath ;" +
    		" psn:thePath ( ns3:controls ns3:srcBrightness ) ] ;" +
    		" a psn:ChangeEffect ;" +
    		" psn:propertyValue \"100\"^^xsd:int ] ] ;" +
    		" ns:hasInput ns1:lampURI ." +
    		" ns1:turnOn a ns1:LightingService , ns3:Lighting ;" +
    		" pvn:instanceLevelRestrictions [ owl:hasValue [ :fromProcess :ThisPerform ;" +
    		" a :ValueOf ;" +
    		" :theVar ns1:lampURI ] ;" +
    		" a owl:Restriction ;" +
    ...
    private static final String SREQUEST="@prefix owl: <http://www.w3.org/2002/07/owl#> ." +
    		...
    		" pvn:requiredResult [ a ns3:Result ;" +
    		" ns3:hasEffect [ :affectedProperty [ a :PropertyPath ;" +
    		" :thePath ( ns2:controls ns2:srcBrightness ) ] ;" +
    		" a :ChangeEffect ;" +
    		" :propertyValue \"100\"^^xsd:int ] ] ;" +
    		" pvn:requestedService [ a ns2:Lighting ;" +
	...
	if (resp != null) {
	    ...
	    if (index > -1) {
		Assert.assertTrue(
			"Received response is not serviceSpecificFailure (because of network error)",
			resp.substring(index).trim().startsWith(RemoteAPI.KEY_STATUS+"="+
					CallStatus.serviceSpecificFailure.toString()));
	    }
	}
    }
