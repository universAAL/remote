...
public class PushGCM {
	...
	public static void pushContextEvent(String callback, String id, ContextEvent event) throws PushException {
		...
		StringBuilder combined = new StringBuilder(serial).append(subject).append(predicate).append(object)
				...
				.append(ContextEvent.PROP_RDF_OBJECT).append("method=SENDC").append("to=").append(id);
		...
		// suffixed as the key
		build.addData("method", "SENDC").addData("to", id).addData(ContextEvent.PROP_RDF_SUBJECT, subject)
				.addData(ContextEvent.PROP_RDF_PREDICATE, predicate).addData(ContextEvent.PROP_RDF_OBJECT, object);
		...
		Activator.logI("PushGCM.pushContextEvent",
	...
	public static void pushServiceCall(String callback, String id, ServiceCall call, String origin)
		...
		if (size < 4000) { // If >4k, dont send the serial, just the inputs
							// (4000 to round up)
			build.addData("param", serial);
		} else {
	...
	private static void send(String callback, String id, Message msg) throws PushException {
		...
		if (callback.equals(REG_ID_OUTDATED)) {// See below for when this
												// happens
		...
		try {
			...
			if (msgid != null) {
				...
				if (res.getErrorCodeName().equals("NotRegistered")) {
					...
					// TODO!!!!!!!!!!!!
					// Activator.getRemoteAPI().register(nodeid,
					// REG_ID_OUTDATED); //This just updates the remoteid
					// Activator.getPersistence().storeRegister(nodeid,
					// REG_ID_OUTDATED, null);
				}
