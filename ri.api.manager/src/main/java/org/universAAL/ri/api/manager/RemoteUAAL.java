...
public class RemoteUAAL extends UAAL {
	...
	public void terminate() {
		// This is like super. , but handles the list of registered CEPs and
		// Profiles
		super.terminate();
		cepsList.clear();
		sprofilesList.clear();
	...
	public class CListener implements ICListener {
		...
		public void handleContextEvent(final ContextEvent event) {
			Activator.getThreadsPool().execute(new Thread("RemoteUAAL_CListener") {
				public void run() {
					try {
						if (event.isSerializableTo(nodeID)) { // MULTITENANT The
																// call is for
																// this scope
						...
						Activator.logE("CListener.handleContextEvent",
								"Unable to send the proxied Context Event to the remote node. " + e.getMessage());
					}
				}
			});
		}
	...
	public class SListener implements ISListener {
		...
		public ServiceResponse handleCall(ServiceCall call) {
			try {
				if (call.isSerializableTo(nodeID)) { // MULTITENANT The call is
														// for this scope
					return PushManager.callS(nodeID, remoteID, call, toURI);
				} else { // MULTITENANT The call is NOT for this scope > answer
							// Denied
				...
				Activator.logE("CListener.handleCall",
						"Unable to send the proxied Service Call to the remote node. " + e.getMessage());
				return new ServiceResponse(CallStatus.serviceSpecificFailure);
			}
		}
	}// TODO Change UAALutils to use async handleRequest also/instead of
		// handleCall, which will allow threading here
}
