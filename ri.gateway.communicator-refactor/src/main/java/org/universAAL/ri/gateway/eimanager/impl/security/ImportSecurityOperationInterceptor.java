...
@Deprecated
public class ImportSecurityOperationInterceptor implements
	ImportOperationInterceptor {
	...
	if (!shouldPass) {
	    throw new InterruptExecutionException(
		    "UID: "
			    + errorUid
			    + " was matched by one or more IMPORT DENY security entries.");
	}
	...
	if (passCount != uids.length) {
	    throw new InterruptExecutionException(
		    "Not all provided uids (only " + passCount + " from "
			    + uids.length
			    + ") was matched to IMPORT ALLOW security entries.");
	}
    }
